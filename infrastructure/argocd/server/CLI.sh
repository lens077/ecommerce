...
# --insecure: 忽略TLS验证
# --grpc-web
#addr=$(kubectl get service example-argocd-server -o=jsonpath='{.status.loadBalancer.ingress[0].ip}' -n $ns)

# HTTP 登录方式
# 凭据只从环境变量来（AGENTS.md 硬规则 4）。这里原本被自动脱敏工具把 CLI 的旗标名
# 也一起涂成 <REDACTED-...>，命令实际已经跑不通——补回旗标，口令仍不落仓库。
addr="argocd-server.app.com"
: "${ARGOCD_PASSWORD:?先导出 ARGOCD_PASSWORD 再跑本命令}"
argocd login \
$addr \
--username admin \
...
# 输出形如 <header>.<payload>.<signature> 的 JWT，直接就是可用凭据——
# 不要粘回本文件或任何文档，用完即撤（argocd account delete-token）。
