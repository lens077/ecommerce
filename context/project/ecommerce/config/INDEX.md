...
- 配置控制面本体：合一仓 `github.com/lens077/control-tower` 的 **`config` 服务**
  （本机 `../control-tower/services/config`；设计见同级仓的 architecture 与 machine-token 设计）。
...
| 热生效边界 | `server` / `discovery` / `observability` 只打 WARN；search 服务的 `search.catalog` 也需重启；`recommend.gorse`（behavior、product）**连 WARN 都没有**，改后必须重启；数据库、缓存和日志级别可立即生效 |
