...
name: cron-jobs
layer: team
description: 定时/周期任务的执行边界——重叠、panic、超时、时区、优雅停止、多实例重复执行与「错过不补」；调度与执行分离（PostgreSQL 任务表 / Kafka）；robfig/cron 与 time.Ticker、K8s CronJob 的选型
---
...
**当前事件主干**：外部非 K8s Apache Kafka 已承载搜索行投影；领域事件的 Outbox Event Router、producer 和 consumer 尚未接线。NATS JetStream 与自写 outbox relay 已退役，不得作为新任务载体恢复。

选哪个看**可靠性要求**，不看有没有 MQ：绝不能丢的任务（订单超时兜底、对账、结算）
仍走 **Postgres 任务表**——它能和 Outbox 同事务写入；其余可重发任务在领域事件链落地后接 Kafka。
不要因为领域事件链尚未接线就退回「一次 cron 回调直接干完」——
...
| Apache Kafka | 跨服务事件与可重发异步链路的目标主干；外部非 K8s 集群，配合 Outbox Event Router + Inbox + DLQ。当前已承载搜索行投影，领域事件仍待接线（§八） |
