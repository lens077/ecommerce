...
- **`es.apikv.com`(rid 47)——⛔ 2026-09-15 退役**:Elasticsearch 已迁入集群 `elasticsearch` 命名空间(仅 ClusterIP),search Pod 直连 `elasticsearch.elasticsearch.svc:9200`,node3 容器 `cdc-elasticsearch` 已删;Pangolin 资源 rid 47 与 Kafka 的 raw 30004 资源待管理员登录后删除(target 已不存在)。以下为历史记录:(2026-09-02 建,同日由 `node3-es` 改名——⚠️ API 只发 `subdomain` 不会重算 `fullDomain`,必须连 `domainId` 一起发,否则 Traefik 仍按旧域名路由、新域名 404(实测); site `node3`=siteId 7, target `127.0.0.1:9200` http, **SSO off**)——Elasticsearch 9.4.5(容器 `cdc-elasticsearch`),机器客户端过不了 SSO,鉴权落在 ES 自身(匿名/错误凭据 401 实测);2026-09-03 已供集群内 search Pod 读取稳定 alias,写入方是 node3 Elasticsearch Sink**
