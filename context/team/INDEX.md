...
| [tls-enablement.md](tls-enablement.md) | 给已在跑的服务补 TLS 的四条检查项与验收法（MinIO 实付学费） | 健康检查硬编码 http 静默失效、服务好着却永远 unhealthy / 整卷挂证书遮蔽原目录且 `:ro` 后建不回来 / 拿公共 CA 证书去配 IP 端点（不签 IP SAN）/ 只测「该通的通了」，配置没生效也照样通 |
