...
| [deploy-parity.md](deploy-parity.md) | 部署清单两份真相源(helm/ 与裸 manifest)必须渲染出同一套集群对象,`scripts/verify-deploy-parity.sh` 强制;共享 SA/CNP/ExternalSecret 只在 `helm/files/` 一份;ESO 占位符文件必须走 `.Files.Get` 不走模板引擎;不再有打包 library tgz;镜像 tag 两边同写 | 只改一边 → 门禁红 / CI 回写只到 helm、集群 apply 裸 manifest → 发版 tag 永远到不了集群(实付学费 2026-09-06)/ ESO 占位符被 Helm 吃成空串 → OTLP 全 401 静默丢 / library tgz 没重打 → 渲染旧模板无报错 |
