...
| PostgreSQL（业务库 + config schema） | `114.132.233.129:30001` | 同左 | node3 Pigsty，PG 18.6，`sslmode=verify-ca` |
...
| Kafka（仅 PoC） | `114.132.233.129:30004` | 同左 | SCRAM-SHA-512；有账号和 topic，**无业务 producer/consumer** |
...
**所有访问都经 node1 `114.132.233.129` 的 Pangolin 隧道**——直接探 `211.144.221.229:5432`
...
`ipBlock: 114.132.233.129/32` + `port 30001`（已在 `backend/tools/outbox-relay/deploy/dev/deployment.yaml` 补好）。
