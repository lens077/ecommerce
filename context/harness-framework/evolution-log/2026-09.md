...
### 2026-09-18 新增阻断式 KYAML 门禁：把「记得给值加引号」换成格式本身的要求

- **改了什么**：新增 `scripts/verify-kyaml.sh`（三段：静态文件逐字节等于 `yamlfmt -o=kyaml` 输出 /
  豁免清单必须**没被**转换 / `helm template` 的每个对象都以 `{` 开头），接进 `scripts/verify-quick.sh`
  的部署清单泳道（排在 parity **之前**）与 CI `deploy-consistency.yml`（`yamlfmt` 钉 `@v1.6.0`）。
  把 79 个部署清单与 helm 模板体全部转成 KYAML，`helm/files/zero-trust.yaml` 列为永久豁免。
  连带改三处按 YAML 文本形状做断言的地方：`scripts/promote-release.py` 的回写正则、
  `scripts/test-promote-release.py` 的漂移注入、`backend/structcheck` 的
  `TestHelmSharedTemplateUsesEcommerceNodeSpread`（后者改成对引号容忍的正则，不再钉死写法）。
  新增 `context/team/kyaml-manifests.md`，AGENTS.md 与 runbook §0.1 各加一条路由。
- **为什么**：YAML 的隐式类型转换出错时**没有任何报错**——文件合法、apply 成功、语义已经变了。
  本仓两笔都已实付：`defaultMode: 0400` 被 kustomize（YAML 1.1）读成 256、被 yq（1.2）读成 400，
  只能绕道写十进制；VPA `updateMode: Off` 不加引号会被解析成布尔 `false`，让 VPA 从「只出推荐值」
  变成「真去改 Pod」。两次的绕法都是「记得手动加引号/写十进制」，而记性不是门禁。
  KYAML 要求值一律双引号，把这类错从「靠人记得」变成「格式本身要求」。
  门禁排在 parity 之前是因为格式坏掉时两道都红，但 parity 只会说「哪个字段不一样」，
  KYAML 那条能直接指出哪个文件写法不对。
- **触发事故**：用户读到 Kubernetes 推广 KYAML 的报道后要求扫查本仓 values 的隐式类型转换风险。
  扫 186 个标量值，经典危险裸值零命中，但发现 `global.consul.enabled` 写成字符串 `"false"`
  而其余 `enabled` 全是布尔——当时消费点只有 `| quote` 注入 env 所以行为正确，可 Go 模板的 `if`
  只看空与非空，非空字符串 `"false"` 恒为真，一旦有人加条件渲染就会静默走错分支。
  转换过程中另实测出两个必须写进规范的坑：①`yamlfmt` 解析失败时**仍返回 rc=0**，只往 stderr 写
  `error decoding`，按退出码判断会把失败当成功；②vite-plus 的 `vp fmt` 也格式化 YAML，
  一次 `pnpm ready` 就把 `frontend/` 下 9 个清单重排掉，而 `backend/` 下同类文件毫发无损——
  两个 formatter 互相推翻，已在 `frontend/vite.config.ts` 的 `fmt.ignorePatterns` 里排除。
- **怎么验证的**：门禁做了双向红测——把一个清单改回块式 → `kyaml[静态]` 红并点名该文件，恢复即绿；
  把豁免的 `zero-trust.yaml` 转成 KYAML → `kyaml[豁免]` 与 `kyaml[渲染]` 同时红（后者因为 `tpl`
  遇到被转义的 `\"` 报 `unexpected "\" in operand`，helm 渲染整个失败），恢复即绿。
  全量验收：`scripts/verify-kyaml.sh` 绿（79 静态 + 1 豁免 + 渲染 56 个对象中 44 个 KYAML、
  其余 12 个全部来自豁免来源）、`scripts/verify-deploy-parity.sh` 绿（pre 56 / prod 54 个对象逐字段
  等价，证明转换语义无损）、`scripts/verify-quick.sh` rc=0、`scripts/verify-context.sh` 绿。
  ⚠️ 期间踩到一次假绿：`scripts/verify-quick.sh 2>&1 | tail -25` 的退出码是 `tail` 的，
  一直显示 0 而实际红；判断成败必须取脚本本身的 rc。
