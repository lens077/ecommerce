...
| 齿 | 文章的坑 | 本仓对应物 | 判定 |
|---|---|---|---|
| 信号 | 弱评估器：LLM 打分有系统性偏差 | 「放行以命令真绿为准，不以模型自报为准」；LLM 评审只在异构双审（runbook §5），跨模型族 + 只读子代理隔离（生成/评估分离） | ✅ 规避 |
...
| 记忆 | 治理 > 存储：版本/溯源/冲突/预算 | git（commit message 写事故与验证）+ 改动处注释；「同一约束只写一处」；AGENTS.md 14KB、TODO.md 96KB 预算门禁；INDEX 分层渐进披露 | ✅ 规避，强于文章基线 |
| 记忆 | 负空间知识（什么不能做）难捕获 | experience 的「关键陷阱」段由门禁机械强制 | ✅ 规避 |
| 记忆 | 时效衰减：过期知识没人清理 | 「过期知识比没有知识更危险」只靠读到时修，是被动的 | ⚠️ 部分踩中 → 进审计清单 #3 |
| 落地 | 版本化一切：变更日志 + 来源 + 关联验证 | commit message 与改动处注释写明触发事故，且「没有事故就别改规则」严于文章 | ✅ 规避 |
...
5. **同构事故检测**——`git log` 近期 fix/harness 提交里同一失效模式是否第三次出现；是 → 规则没治本，升级为机械约束
