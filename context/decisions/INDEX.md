...
git 历史分工明确：commit message 按时间追加、永不改写，
记「触发事故 + 怎么验证的」（evolution-log.md 已于 51374c92 删除，编年史只留在 git）；这里的 `implemented/` 随交付事实**同步改写**，读者不必从
几十条日志里自己拼出"现在是什么"。
...
3. **随交付事实改写，不追加历史**。代码后来搬了路径、改了键或默认值，同一次提交改这里的事实；
   "原本 / 现在 / 不再"这类叙述归 git（commit message）。**决定本身被推翻时不改写旧文件**：
