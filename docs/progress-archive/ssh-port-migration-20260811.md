...
layer: team
description: SSH 改端口在 Ubuntu 24.04(socket activation)下的正确姿势与 2026-08-11 node1 VPS 锁死实录——改 sshd_config 的 Port 无效、ListenStream 纯端口号只绑 IPv6 导致 v4 全断、cloud-init 首值覆盖 PasswordAuthentication
---

> 📦 **归档记录（原 `context/team/ssh-port-migration.md`，2026-08-24 归档）**
>
> 这是一次性的主机运维实录：2026-08-11 把两台公网 VPS(node1/node2)的 SSH 端口从 22 改到
> 34123/34124，中途把 node1 锁死后靠腾讯云 VNC 救回来的全过程。
>
> **为什么移出 `context/team/`**：团队层的判据是「换个模块、换个服务依然成立」，
> 而本文绑定在两台具体 VPS 上，且前提 Ubuntu 24.04 的 socket activation 行为与当前
> 集群实况(Ubuntu 26.04)已不一致，不适合再当作现行规范被 agent 直读。
>
> **仍然有效的部分已被提炼走**：「Connection refused vs timeout」这条通用判别，
> 以及「动远程访问配置前先确认带外通道」，已并入
> [`context/team/pangolin-tunnel.md`](../../context/team/pangolin-tunnel.md) 的排错一节。
>
> 保留本文是为了历史可追溯（尤其是锁死当时各条带外通道的实测可用性），不作为操作指引。
> 真要再改 SSH 端口，先在目标机上核实 systemd 版本与 socket activation 行为。
...
| 腾讯云 TAT 自动化助手(tccli 免 SSH 下发命令) | ❌ 当时本机无 tccli 凭据;**事后已配好并实测,现为首选带外通道**(实例信息见 [pangolin-tunnel.md](../../context/team/pangolin-tunnel.md)) |
