...
> `docs/reviews/ADVERSARIAL_REVIEW_20260806.md`。
...
- **Protobuf + Buf + Protovalidate**：13 个 API 包的 14 份 proto 已定义，并按 SQL 契约补齐字段与跨字段校验；Go/TypeScript 生成链路及运行时正反例测试已打通
...
### 5.2 工程化规范 📐
- 统一的目录结构与代码规范
- 项目级 Codex 规则已落地：`.codex/config.toml` 配合 `AGENTS.md`，在全自动会话中自动处理权限请求，同时为会实质改变结果的互斥决策保留交互式选择框
...
> 详见 `docs/reviews/ADVERSARIAL_REVIEW_20260806.md`。
...
| 2026-08-07 | v1.13 | **修复九个服务走配置中心时的 401**。address/behavior/inventory/merchant/order/payment/product/search/user 的共享配置源统一迁到独立 `config-center/sdk/configsource`，不再手写 ConnectRPC client；SDK 从本地 selector 的 `config_center.service_token` 设置 `x-config-center-service-token`，定向测试以真实 Connect handler 断言该 header。旧 `CONFIG_SOURCE=configcenter` 现在明确失败，避免无 token 的旧路径复活。九个 Makefile 与 cart 对齐：默认 `make dev` 读取被忽略的 `configs/source.dev.yaml` 并选择配置中心，`make dev-consul` 保留历史 KV 路径；新增不含 token 的 selector 示例，生产迁移须以 Secret 文件挂载。18 份既有 deploy 清单仍选 Consul，旧直连变量说明改为 SDK selector Secret 要求。定向配置包测试、`go build ./...` 与 `go vet ./...` 均通过 | - |
| 2026-08-07 | v1.13 | **确立 `context/team/runbook.md` 为 Codex 的单一入口（纯文档，无代码改动，完成度不变）**。此前 runbook 只有「跑什么命令」，没有「动手前该读哪份规范」，新增的 `go-redis.md` / `cron-jobs.md` 与两份工程体系文档也没有任何入口指向它们——Codex 只自动读 AGENTS.md，等于读不到。补法是在 runbook 加 §0.1「按改动类型的必读路由」：9 行表覆盖服务拓扑、proto、Redis、定时任务、指标告警、CI-CD/部署、本地环境、提交、踩坑沉淀，每行写清「先读哪份 + 不读会怎样」（如「扩副本后同一任务跑 N 次」「抓到已 Close 的旧客户端」），**只给指针不复制内容**——同一约束写两处必然漂移（`knowledge-layering.md`）。并显式标注 `DEVOPS.md`/`OBSERVABILITY.md` 是目标态文档，读它们是为了不把新代码写歪，**不代表对应能力已存在**，现状一律以 `TODO.md` 为准。两份 AGENTS.md（根 + ecommerce，链接各自相对正确）的 runbook 指针同步改写为点明 §0.1 | - |
| 2026-08-07 | v1.12 | **新增团队级定时任务约定 `context/team/cron-jobs.md`（纯文档，无代码改动，完成度不变）**。以一篇 robfig/cron 文章为骨架，但**本仓根本没引这个库**——周期任务全是 `time.Ticker`（Consul 心跳 ×11 同构、`product/internal/biz/recommend.go:122` 的 gorse 增量同步、`behavior/internal/biz/usecase.go:268,312` 的 flush/retry、网关健康检查与策略刷新），所以文档写成「Ticker 与将来的 cron 共同遵守的执行边界」并给了选型判据（单一固定周期继续用 Ticker，多条时间规则才值得引库，可独立起容器的对账/清理首选 K8s CronJob）。最值钱的一条是**多实例重复执行当前只是被单副本掩盖**：`DEVOPS.md` 阶段 2 要求无状态服务扩到 ≥2 副本，那天起所有进程内定时任务会立刻跑 N 次——列为扩副本的前置条件，并强调锁只是优化、幂等才是底线。另把两处本仓真实事故写进对应小节：Ticker 首次触发要等满一个周期（Consul TTL 25s 盲窗、网关 503「刷几次才出数据」的根因）、长生命周期任务挂 `OnStart` 的 ctx 会立刻退出（inventory 心跳被 Consul 判死摘除），均链到 experience 与 TODO。第八节按「Cron 只发信号 → 任务表/消息 → 幂等 worker → 可重跑」重述了 TODO 已规划的超时兜底 job、支付每日对账、库存对账，并注明 Kafka 依赖为 0 时先落 Postgres 任务表、不要退回「一次回调直接干完」。已登记进 `context/INDEX.md` 与 `context/team/INDEX.md` | - |
| 2026-08-07 | v1.11 | **新增团队级 Redis 使用约定 `context/team/go-redis.md`（纯文档，无代码改动，完成度不变）**。以一篇 go-redis 文章为骨架，但按本仓实况改写而非照搬：本仓已做到「启动建一次 + 依赖注入」，**更严的约束是客户端会热重建**——`LiveRedis` 只暴露 `Client()`，把返回值存进结构体字段迟早抓到已 Close 的旧客户端。另落七条：`redis.Nil` 是未命中不是故障（正例 `product/internal/data/recommend.go:178`）；Pipeline 结果只在 `Exec()` 后有效、错误从 `Exec()` 出、无原子性（正例 `behavior/internal/data/event.go:150` 的 SetNX 去重）；**默认 `MaxRetries:3` 让非幂等命令可能执行多次**——与网关重试放大是同一类坑，链到那份 experience；读-改-写用 `Watch`+`redis.TxFailedErr` 重试循环，并就 TODO 里计划的「inventory Redis 分布式锁」写明 owner+TTL+Lua 解锁、且不能替代 SQL 侧 `available >= @quantity` 校验；Cluster 迁移的跨槽多键与无多 DB 两个坑；Redis 不顶替 Kafka（本仓 EventBus 仍是进程内、Kafka 依赖为 0，明确禁止「先用 Redis 顶一下」）。标注两处待改：TLS 走 `insecure_skip_verify` 只加密不校验身份；连的是 Dragonfly 不是原生 Redis，Cluster/Stream 用前需实测。已登记进 `context/INDEX.md` 与 `context/team/INDEX.md` | - |
