...
- [ ] **node1 PostgreSQL 对全网开放 + 明文 + `msdnmm` 弱口令** — 比 Redis 更糟（Redis 至少已上 TLS + 40 位随机密码）。同一张 `*.apikv.com` 证书可直接用于 `ssl_cert_file`，改法参照已完成的 Redis 那条；密码一并轮换。<sub>TODO L445</sub>
...
- [x] ~~**MinIO/gorse/harbor 经 Pangolin 暴露 + node2 端口全部收回回环**~~ — ⚠️ 沉淀的硬约束：`8.138.194.254` 是阿里云机、`apikv.com` **未在阿里云备案**，经该域名访问必被网络层拦掉，**"配域名+证书直连"这条路根本走不通**，唯一解是让公网流量落到 node1 再经隧道回来。<sub>TODO L405-407, L444</sub>
...
> ⚠️ 命名歧义提醒：`node3` 在本仓有**两个不同所指** —— ①集群内节点 `192.168.3.107`（上述已删条目说的是它）②Pangolin 那台 VPS `114.132.233.129`（旧称 node3，已于 2026-08-18 统一改称 node1，见 `context/team/pangolin-tunnel.md`）。读到 node3 先分清是哪个。
