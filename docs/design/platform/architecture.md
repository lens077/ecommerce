...
- 异步事件通信：目标形态使用 PostgreSQL outbox、relay 和 NATS JetStream 发布领域事件，消费者以幂等处理实现最终一致性。当前只有商品索引 worker 与基础设施完成部署，不能把下表其他事件视为已接线。
...
| ProductChangedEvent   | SPU/SKU_ID、完整搜索投影、变更时间         | 商品服务 | search indexer           | 通过 outbox 与 NATS 同步商品投影至 Meilisearch；生产者尚未接线 |
