...
| MinIO（集群外） | — | — | `https://minio.apikv.com:9000`（=node2）；管理台 9001 已绑 `127.0.0.1` | ✅ TLS（ZeroSSL `*.apikv.com`，**2026-10-27 到期**，2026-08-19 落地） |
| gorse（集群外） | — | — | **node2**:8088 / :8086，公网明文 | ❌ 公网 http（实测 8088 匿名 200） |
...
4. **公网明文最优先**：casdoor（OAuth code/token）与 node2 的 gorse 仍走公网 http，
