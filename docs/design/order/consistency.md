...
1. **可靠投递底座（必选）**：本地事务 + **Outbox 表 + 持久事件主干**。写订单与写 outbox 同一事务；Debezium Outbox Event Router 从 WAL 搬运到外部非 K8s Kafka，发布进度由 WAL/Connect offset 表示，不维护 `published` 簿记。NATS 与自写 relay 已退役。
...
3. **B 段·阶段性终态后的副作用（最终一致）**：走去中心化编舞。Order 编排器将核心流程状态持久化，并经 Outbox 发 Kafka 领域事件；支付回调与库存结果可作为编排器输入，履约、通知、分析等下游独立消费。搜索投影走独立 CDC 线，不消费领域事件。取消/超时的核心补偿仍由 Order 编排器显式驱动，不能只期待消费者自行收敛。
...
- **事件可靠性**：Topic 按限界上下文划分，`aggregate_id` 作为 partition key；消费者按可重试/永久失败分类定义重试和 DLQ，并对停滞与死信告警，不沿用 JetStream `MaxDeliver`
