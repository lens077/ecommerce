...
- frontend 修复后，剩余 deny 全部是 product 周期性访问已退役的 `8.138.194.254:8088`。这是下节 Gorse Config Center 阻塞的同一真实信号，因此 `EcommerceNetworkPolicyDeniedBurst` 保持 firing；没有通过放行旧 IP、调高阈值或静音掩盖它。
...
- product 的 dev Config Center version 3 仍指向已退役的 `8.138.194.254:8088`，product/behavior 的 Gorse API key 仍为空。当前 Gorse endpoint 健康、node2 配置中的真实 key 存在，但 machine token 按设计只有 `GetKey`/`WatchKeys`，管理员写入需要 Casdoor JWT；用户本轮暂时无法登录，因此没有绕过 Config Center 或直接写数据库。
