...
- [ ] **完成 Gorse Config Center 更新**：管理员登录后把 product endpoint 从已退役 `8.138.194.254:8088` 改为 `https://gorse.apikv.com`，为 product/behavior 写入正确 API key，滚动并验证 Track/Recommend/SimilarItems；不得放行旧 IP 或绕过 Config Center 直写数据库。
