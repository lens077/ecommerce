...
>
> ⚠️ **本文件里的 36 个复选框全部是「新项目」的验收模板，不承载本仓进度**（§十 已写明
> 「复制到新项目的 `TODO.md` 里逐条勾」）。本仓的进度与待办唯一真相源是
> [`TODO.md`](../TODO.md)，明细在 [`docs/todo/`](todo/README.md)。
> 2026-08-29 加注：机械扫描 `- [ ]` 会把本文件误判为并行进度源，已因此重复审计过一次。
