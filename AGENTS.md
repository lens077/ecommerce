...
3. **提交前先更新 `TODO.md`**，再 `git commit`。提交信息走 Conventional Commits
   `<type>(<scope>): [:emoji:] <subject>`，由 `.husky/commit-msg` + `commitlint.config.mjs`
   强制校验：type 限十一类，emoji 可选但必须与 type 相符，subject 末尾不加标点。
...
- 前端：pnpm workspace（`frontend/apps/{consumer,merchant,admin,config,desktop}` + `frontend/packages/`），React 19 + MUI 9 + TanStack + Connect-RPC；`desktop` 是 Tauri 壳，复用前三个 app
- 工程化：仓库根另有一个只装 commitlint + husky 的 `package.json`，与 `frontend/` 的 workspace 相互独立
