...
8. **解决问题优先，不要为了堆工作量去写测试**。测试的唯一理由是「它能验证这次改动是对的」或「它能挡住这个 bug 复现」，不是「看起来干了很多活」。
   - **该写的照写**：修 bug 时补一条能复现该 bug 的回归测试；改动核心逻辑或边界条件时补断言。这类测试是验证手段，属于最小路径。
   - **不要写的**：为覆盖率凑数的测试、断言 getter/setter 或框架行为的测试、把已有实现照抄一遍的「镜像测试」、大批量表驱动样例但只换无关字段的测试。
   - **不要顺手扩张**：用户要求修 A，不要顺带给 B、C、D 补测试再来邀功；想扩张先说明理由并征得同意。
