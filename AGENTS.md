...
执行授权 ≠ 消费授权。几十 token 的输入可以在后台变成几百次付费决策，用户看不见也拦不住（5 分钟 1000 元的事故见 e3-execution.md）。E3 管「读多少」，本节管「花多少」：
...
放行以「命令真绿」为准,不以模型自报为准。核心改动 push 前做异构双审（两个只读 subagent 分走不同模型族审 diff，runbook §5）。
