...
| 告警卫生 | 🟡 | **`ecommerce-k8s.yml` 点号→下划线迁移已完成并双向验收**〔实测 2026-08-31：6 条规则先在 VM 查到 series 再改写，双写 + dry-run + reload；兜底 `K8sClusterMetricsMissing` 由慢性 firing 回 inactive，条件规则复明后数分钟内当场捕获真实故障；另修 `K8sContainerNotReady` 的 Completed-Pod 误报类〕；**文档侧滞后已于 2026-09-01 清理**〔根因是 VM 启动带 `-opentelemetry.usePrometheusNaming=true`（VM 2.24.0，实测 2026-09-01）；规则/Grafana 早已迁完，但 5 份活文档仍在教人用点号，其中 `docs/observability/alerting-notification.md` 正是 TECH.md §9.3 指向的口径手册。已统一改为「写查询前先查当前口径」，并记录三个存储口径不同：VM 下划线、VictoriaLogs 点号、VictoriaTraces `resource_attr:` 前缀+点号〕；**剩余噪音**：`PostgresReplicationLag`/`NodeMemSwapped`/`EcommerceNetworkPolicyDeniedBurst` 与若干 `AlertFiringTooLong` 慢性告警每小时推 ntfy 待清——已交接独立会话（见分类文件 P1） |
