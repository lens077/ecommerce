...
- [ ] **部分完成 · GitLab 自建 Runner 验收**：仅保留集群内 Kubernetes executor，单并发且排除 control-plane；轻检查 512Mi、重构建 3Gi，默认预算保持 3Gi。剩余：GitLab 轻检查端到端与自动清理验收；worker 容量满足 3Gi build 加 helper 后再验证 Go/前端重构建，不降低请求绕过容量限制。
