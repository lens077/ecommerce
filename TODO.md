...
- [x] **四个集群服务经隧道公网暴露（2026-08-27）**：`argocd`(rid 31) / `consul`(rid 32) / `search`(rid 33) / `cart-api`(rid 34)，**全部 SSO on**——是控制面，登录墙是主要防线。两步法：HTTPRoute **追加** `xxx.apikv.com` hostname（保留原 `dev.test`，增量可回退）→ Pangolin 建资源指 `10.110.51.106:443 https`（k8s 站点 siteId 4 的资源全部共用这个 target，那是 `cilium-gateway` 的 ClusterIP，分流靠 hostname）。
  ...
  - ⚠️ **08-24 helm uninstall 留下四条孤儿 HTTPRoute**：`observability`(grafana/jaeger)、`logging`(loki)、`victoriametrics`(vm-single) 的命名空间里零 Deployment、endpoints 无后端，给它们建公网入口只会得到 502。**建资源前先查 `kubectl get endpoints`**。这四条路由待清理。
