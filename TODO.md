...
| cart 三配置源 | 🟡 | `cart/internal/pkg/config` 现支持 `CONFIG_SOURCE=file\|consul\|configcenter`，不做失败自动降级；本地文件与 Consul 保持启动加载语义，配置中心读取/Watch 使用只读服务 token。独立仓的 `api/sdk/v1/source.proto` 提供最终的 `SourceConfig` 三选项契约，待 v0.1.0 发布后替换主仓兼容层 |
...
| 配置中心 Go 客户端 SDK | 🟡 | 独立仓已提供 `sdk/configsource` 和 Proto `SourceConfig{file, consul, config_center}`；待生成契约、发布 v0.1.0，并将 Cart 由兼容层切到版本化 module |
| 审批/灰度/密钥加密/审计 | ⬜ | 后续阶段 |
