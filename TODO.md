...
- [ ] **PG 切流未做（有依赖链风险，需决策）**：10 个服务仍连 CNPG。真要切，`config` schema（Config Center 自己的数据）也跟着走 → control-tower 的 config 服务要连 node3，而它是 10 个服务的 bootstrap 来源，**node3 或隧道一断整个平台起不来**
...
- [ ] **control-tower 网关指标发错地方**：日志刷 `failed to send metrics to http://jaeger.observability.svc:4318/v1/metrics: 404`——把**指标**发到了 Jaeger 的 OTLP 端口，应指向 otel collector（`otel-opentelemetry-collector.opentelemetry.svc:4318`）
