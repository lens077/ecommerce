...
- [x] **可观测彻底挪出内网（2026-08-24）**：node3 加 nginx `/v1/*` 路径改写（`otlp-ingress.conf`，Pangolin resourceId 30 → `node3-otlp.apikv.com`），十个服务 + 网关的 OTLP 直连它；随后 **helm uninstall + 删 PVC** 清掉 grafana/jaeger/loki/vl/vm-single/fluent-bit/otel-collector，回收约 23Gi，只留 vector 送容器日志。⚠️ 查 node3 指标时标签名带点（`service.name`/`k8s.container.*`）不是下划线
