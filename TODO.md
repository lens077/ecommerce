...
- [ ] **PG 切流卡在写不了 Config Center**：数据已迁完，但服务的 `data.database.postgres` 存在 Config Center，而 `PutKey` **只认 Casdoor 管理员 JWT**（machine token 是只读，仅 `GetKey`/`WatchKeys`，见 control-tower `machine-token.md`）。拿到管理员 JWT 即可改 10 个服务的 bootstrap。依赖链风险仍在：`config` schema 若一并搬走，config 服务就要连 node3，而它是 10 服务的 bootstrap 来源——建议 config 留在 CNPG，只搬业务库
- [ ] **Silo 缺 Pangolin 入口**：`9000/9001` 只在 node3 本机可达。参照 node2 minio 资源（target `127.0.0.1:9000`、method `https`），但**别照抄它的 TLS 配置**——Silo 用自签 `sss.pigsty` 证书而非 `*.apikv.com`，回源要设 `tlsServerName` 或跳过校验。且 Pangolin 里 `apiKeys` 表 0 行，只能在面板手工建
...
- [ ] **把 otel-collector 也挪出内网（卡 Pangolin）**：两条路——① node3 装 collector 经 Pangolin 暴露 raw TCP；② node3 nginx 加 `/v1/{traces,metrics,logs}` → Victoria 各自路径的反代，作为 HTTP 资源暴露成 `node3-otlp.apikv.com:443`（符合 host_and_port）。都要先能在 Pangolin 建资源
