...
- [ ] **待触发 · Outbox 结构与搬运**：`pkg/outbox` 零调用方已删，生产者随首个事件重写；`products.outbox` 表仍在，补 `aggregate_type`、清理旧发布簿记列；Debezium Outbox Router 使用独立 publication/slot，与搜索 CDC 分线。配置 `acks=all`、幂等 producer 及复制槽保留策略，不自写 relay。
...
- [ ] **未完成 · 事故记录固定证据来源**：`context/**/experience/*.md` 已在做事后沉淀但结构不固定，回头统计「哪类故障靠什么证据定位」做不到。在 `context/harness-framework/self-refinement.md` 的 experience 规范里固定五段：触发告警 → 可复跑的查询语句 → 证据（每条标 log / trace / metric / blame 来源）→ 反向证据（排除了哪些假设）→ 结论；`verify-context.sh` 的 experience 格式检查随之加断言。这些记录也是将来自动排障评测集的原料。
...
- [ ] **部分完成 · 推荐登录身份关联**：登出重置匿名标识已完成（tracker `resetIdentity`，consumer 登出调用）。剩两项且有先后：① 上一条「网关可选认证路由」；② behavior 新增 `identity_links` 表，同一请求同时带网关用户 ID 与 `anonId` 时记关联（一个 anonId 只关联首个用户），按 Postgres 汇总值 PUT 回填 gorse（幂等可重试），dislike 过滤纳入已关联的匿名 ID。gorse 无用户合并 API，只能在 behavior 做。
