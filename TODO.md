...
| 节点优雅关机约定（`context/team/node-graceful-shutdown.md`） | ✅ | 2026-08-21 新增团队级规范（INDEX×2、runbook §0.1、local-env 已接线）：节点 `shutdown now`/reboot 后最多等 90 秒是 kubelet GracefulNodeShutdown 的设计预算（90s 总量、其中 30s 留给关键 Pod），… |
