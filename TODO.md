...
- [ ] **公网明文端点（优先级高于所有集群内明文）**：①casdoor `apikv.com:8000`（=114.132.233.129）承载 OAuth code/token 交换，**走公网 http** ②`8.138.194.254:8088`（gorse API）与 `:8086` 同样公网明文裸奔，实测 8088 匿名 200。minio 部分已于 2026-08-19 解决，见下条
...
- [ ] **MinIO 收尾三项（承上条，缺一不可）**：①**DNSPod 加 A 记录 `minio` → `8.138.194.254`**——泛解析 `*` 指向 node1 `114.132.233.129`，不加这条精确记录则 `minio.apikv.com` 落到 node1，`cart` 的 dev/pre 配置（已改为 `https://minio.apikv.com:9000`）拿不到图；本机 tccli 凭据无效（`secretId is invalid`），需控制台或新 Key ②**轮换 MinIO root 密码**——`admin` + 8 位弱口令明文写在 compose 里，而 9001 管理台在公网 200 暴露了未知时长，不能假定没被试出来；轮换后同步 compose 与任何 mc alias ③**证书续期现在是三处同步**（blog ssl / pangolin traefik certs / 本机 minio certs），`*.apikv.com` **2026-10-27 到期**且 node1 续期链路缺位（见 `context/team/pangolin-tunnel.md:26`），到期会三处同时静默挂掉
- [ ] **Config Center 的 cart KV 同步 MinIO 新端点**：仓库副本 `backend/services/cart/configs/{dev,pre}.yml` 已改 `https://minio.apikv.com:9000`，但 KV 是另一份（`TODO.md` 的「三份配置对齐」教训）。**必须等上条的 DNS 记录生效后再灌**，否则 cart 热更新会立刻拿到一个解析不到的域名
