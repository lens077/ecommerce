...
| 后端 config 服务 | 🟡 | 已迁往独立仓 `github.com/lens077/config-center`：保留原 Postgres schema，服务自身改由本地 `CONFIG_FILE` 自举，Consul 仅服务发现；新仓增加 Casdoor 管理员 IAM、只读服务 token 与独立 Go SDK。主仓兼容实现将在新模块发布并完成部署切换后删除 |
...
| 项目 | 状态 | 说明 |
|------|------|------|
| 链路追踪（OpenTelemetry/Jaeger） | 🟡 | 后端 11 个服务、Gateway 的 OTel 核心 Trace/Metrics 已统一至 `v1.45.0`；Logs 为 `v0.21.0`，`otelhttp` 为 `v0.70.0`，`otelpgx` 为 `v0.11.1`。11 个 Connect 服务端拦截器均同时启用 `WithTrustRemote()` 与 `WithoutServerPeerAttributes()`：保留可信上游 trace context，避免由服务端写入不可靠或高基数的 peer 属性。Gateway 同步迁到 `semconv/v1.41.0` 的稳定 HTTP 属性键。服务端 `otelhttp` 中间件、ES OTel 传输已接入；Config Center 对齐待其并发观测改动稳定后进行。补记 `WithoutServerPeerAttributes()` 的实际影响：`net_peer_port` 是**按 TCP 连接**取值的，实测 cart 一个 `rpc_server_duration_milliseconds_count` 就有 39 条序列、`net_peer_port` 39 个不同值,每条只有一个样本且永不递增 —— 于是 **`rate()` 恒为 0**，不只错误率，「请求率 by 服务」「P95 时延 by 服务」在改之前都算不出真实值(不是空图,是错的值)。otelconnect 自己的文档也写明该默认行为 "produces very high-cardinality data"。这与 collector CR 里对 pod 名、lxc 网卡做的基数纪律是同一件事,只是发生在应用侧 |
...
| service_name 撞名（config-service） | ⬜ | **两个不同的程序在用同一个 `service_name="config-service"` 往 VM 推指标**：本仓的 `backend/services/config` 与独立仓 config-center，`cmd/server/main.go` 里的默认服务名相同、都绑 `0.0.0.0:30010`、连的还是同一个 `pg-dev.app.com:5432/ecommerce` 池 —— 所以连 `db_client_connection_pool_name` 都区分不开。后果有三层:①任何按 `service_name="config-service"` 过滤的面板(业务盘的请求率/错误率/P95、config-center 控制台的 API 与 DB 图)拿到的都是两个进程的混合值,不是错在「查不到」而是错在「查到了但不是那个服务的」;②`process_*` 看起来干净纯属巧合 —— 只因为旧的那个不含 sysstat;③已经真实导致过一次误判(见上一行被推翻的 sysstat 诊断)。**改法**:先决定旧的 `backend/services/config` 是留是撤 —— 配置中心已独立成仓并打了 v0.1.0,这个服务大概率是要退役的;若要并存,则给独立仓改名 `config-center`(它是新来的,改名影响面小),并同步改 `internal/pkg/promql/catalog.go` 的过滤条件与两处 Grafana 脚本。在改完之前,涉及 config-service 的 API/DB 数字一律不可信 |
...
- [ ] **`service_name` 撞名**：`backend/services/config` 与独立仓 config-center 同名同端口同库，所有按 `service_name="config-service"` 过滤的面板拿到的都是两者的混合值（不是空图，是错的值）。先定旧服务是留是撤 —— 配置中心已独立成仓并打了 v0.1.0，大概率该退役了。详见「可观测性与测试」一节
