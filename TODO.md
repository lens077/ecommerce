...
- [ ] **一致性底座**：落 Outbox 表 + **自写 relay → NATS JetStream**（2026-08-20 选型定稿，原「Kafka relay」表述作废，见下「技术选型定稿」小节），替换现有进程内 `GoEventBus`（跨服务事件当前到不了其他服务）
...
- [ ] **①灾备止血（最高优先，唯一「不做会出事」项）**：4c4G 云箱装 SeaweedFS 单机 S3（备份靶）→ Velero（FSB/Kopia，管 K8s 资源与非 PG PVC）+ CNPG **Barman Cloud Plugin**（管 PG 一致性 PITR，RPO=WAL 5min）→ **age 客户端加密后密文着陆** → 归档年龄监控 + 2c2G 哨兵互拨告警 → **每周恢复演练到隔离 ns**；商品图 MinIO→SeaweedFS 迁移（S3 兼容 PoC：签名/multipart/presigned/生命周期；**新增备份流量即刻不写 MinIO**——上游已归档）
...
- [ ] **③NATS JetStream 落地**（TECH-RADAR §1 定稿）：helm 3-server（meta R3）+ NACK CRD 进 ArgoCD → stream 分级副本（**交易域 R3、埋点 R1**）→ outbox 表按 CloudEvents 属性设计 + 自写 relay（复用 pg_notify 经验，`Nats-Msg-Id=outbox_id`）→ 首接消费者=search 索引喂养 → **交易事件接入前置=R1 故障→outbox 积压重放演练**；季度演练：`prlctl stop` 单 VM 验选主+重放+CNPG switchover；随后退役 Strimzi/Kafka/Debezium 部署
...
- [ ] **⑬资源与残留**（预算=对抗第 2 轮 T2）：~~先杀残留~~（🟡 2026-08-20 部署测试时已清：seata ns/cilium-test-1 ns/strimzi/tempo/集群内 minio，node2 内存 77%→48%；剩 loki+fluent-bit(随④切主)/dragonfly 残留/consul(随⑤)）；全栈 requests 目标 ≈13Gi/19.5Gi（余量 ≥20%），limits=1.5×requests（CH/网关 2×），requests 按 VPA 实测校准；装不下按砍序：残留→Tetragon(已缓)→Kyverno audit-only→Jaeger 采样→CH 降档→KEDA→OpenFGA 2→1
...
- [ ] **[两仓] 重建索引时一次洗掉三笔历史债（不修则迁完仍排不了序）**：①价格字段在旧 ES 索引里是 **text**（`backend/infrastructure/kafka-connect/kafkaconnector.yaml:46` 注释自认），Meilisearch 索引必须建成数值型并声明进 `sortableAttributes`/`filterableAttributes`；②字段契约错位从未对齐（呼应上文「代码与设计不符」既有记录）——实现读 `id`/`skus[].price`/遍历 `sale_detail[]` 累加，设计稿 mapping 写 `spu_id`/顶层 `price`/`sale_count`，借新索引 schema 一次定稿，同步改 `search.go` 解析与 `docs/design/search/search.md`；③金额类型定型（int64 分 或 decimal string 二选一）——与 TECH-RADAR 1.6 CloudEvents 事件契约是同一笔债的两面，索引 schema 与事件 schema 必须同一口径
- [ ] **[本仓] `SearchRequest.index` 从前端收回服务端**：索引名由前端硬编码传入（`frontend/packages/constants/src/search.ts:1` 的 `SEARCH_INDEX="ecommerce_products_spus"`），等于把索引选择权暴露在公网免鉴权接口上。迁移时索引名收进 search 服务配置，proto 的 `index` 字段标 `deprecated`（不删字段不复用字段号），前端删常量与传参
