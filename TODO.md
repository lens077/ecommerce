...
2026-09-12 补齐生产交付代码：新增 prod Helm/kustomize 独立基线，dev/pre/prod parity 全绿；两个前端在原生 amd64/arm64 runner 构建，发布 tag 全量包含 search，晋级脚本校验十二个多架构 index 并成对固定版本与 digest；部署入口要求显式生产 context。原基础设施 P1 收窄为「首次真实 CI 发布与 prod 晋级验收」，未关闭；本轮未推 tag、未发布制品、未修改线上。操作入口 [PRODUCTION-RELEASE.md](docs/PRODUCTION-RELEASE.md)；
