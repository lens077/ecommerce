...
- [ ] **一致性底座**：落 Outbox 表 + **自写 relay → NATS JetStream**（2026-08-20 选型定稿，原「Kafka relay」表述作废，见下「技术选型定稿」小节），替换现有进程内 `GoEventBus`（跨服务事件当前到不了其他服务）。**2026-08-21 底座代码已落地并本地端到端验证**（见「③NATS JetStream 落地」行的进展注），剩 order 侧接线与集群部署
...
- [x] **[两仓] 重建索引时一次洗掉三笔历史债（不修则迁完仍排不了序）**——**2026-08-21 已随新索引 schema 定稿**（`backend/pkg/searchindex` 的 `Doc`）：①price 建成**数值型**并进 `sortableAttributes`（filterable=status/merchant_id，sortable=price/sale_count/updated_at）；②字段契约对齐设计稿口径：顶层 `id`(=spu_id)/`price`/`sale_count`，事件 payload 即文档投影（event-carried state），索引与事件天然同一口径；③金额定型=**数值投影**（真相仍是 PG DECIMAL，索引值仅展示/排序，不参与金额运算——对抗第4轮记录了 int64 分的备选与取舍）。**尚欠**：`search.go` 服务侧解析与 `docs/design/search/search.md` 待随上面「search 服务 ES→Meilisearch」一并改
