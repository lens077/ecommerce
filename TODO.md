...
| 项目 | 状态 | 说明 |
|------|------|------|
| 2026-08-24 前端切 BFF 会话（P2，Web 端完成） | 🟡 | 鉴权模型翻案为 **BFF + 服务端 session**（决策见 control-tower `docs/design/adr-0002-bff-session.md`，手顺见 `bff-migration.md`）。Web 端不再持有任何令牌，续期在服务端完成。证据与设计要点见 [归档](docs/progress-archive/2026-08-24-bff-session-migration.md)。**待办**：merchant/admin 复用 `bff.ts` 接入登录；pre/prod 需去掉 `SESSION_COOKIE_INSECURE` 并设 `Domain=.apikv.com` |
| 2026-08-24 桌面端切会话轨（P3，代码完成待真机验证） | 🟡 | Tauri 端改用同一套服务端会话，网关 `mode=native` 经回环回调交回 session id（**Rust 侧一行未改**）；两端登录路径就此合并，PKCE 退场。顺带修了路由守卫的隐性断裂（详见 [归档](docs/progress-archive/2026-08-24-bff-session-migration.md)）。**待办**：桌面端真机跑一次；通过后删 `pkce.ts`/`tokenStore.ts` 与 `/callback` 路由 |
