...
| 配置中心 Go 客户端 SDK | ⬜ | 后续：把 `internal/pkg/config` 抽成共享包，避免 10 份复制（现在是 10 份逐字相同的副本，只有 import 路径不同） |
| 审批/灰度/密钥加密/审计 | ⬜ | 后续阶段 |
