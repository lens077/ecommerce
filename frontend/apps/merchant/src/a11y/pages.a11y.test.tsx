/**
 * merchant 关键页 a11y 断言：axe（WCAG A/AA）+ 标题大纲（唯一 h1 + 不跳级）。
 *
 ...
 *
 * 跳过 /reports：它挂 ECharts（canvas），jsdom 没有；标题语义已改，交给手动走查。
 */
...
vi.mock("@ecommerce/configs", async (importOriginal) => ({
  ...
  fetchIdentity: async () => ({ authenticated: true, roles: ["merchant"], name: "测试商家" }),
}));
...
beforeAll(async () => {
  await initI18n({ ns: "merchant", resources: { "zh-CN": merchantZh, en: merchantEn } });
});
