...
function DashboardPage() {
  const stats = [
    { label: "今日订单", value: "128", change: "+12%", trend: "up", icon: ShoppingBag },
    { label: "今日销售额", value: "¥12,580", change: "+8%", trend: "up", icon: DollarSign },
    { label: "待处理订单", value: "24", change: "-3%", trend: "down", icon: Package },
    { label: "客户总数", value: "1,234", change: "+5%", trend: "up", icon: Users },
  ];

  const recentOrders = [
    { id: "ORD001", customer: "张先生", amount: "¥299.00", status: "待发货" },
    { id: "ORD002", customer: "李女士", amount: "¥1,280.00", status: "待发货" },
    { id: "ORD003", customer: "王先生", amount: "¥89.00", status: "已完成" },
    { id: "ORD004", customer: "赵女士", amount: "¥456.00", status: "待发货" },
    { id: "ORD005", customer: "刘先生", amount: "¥2,100.00", status: "已完成" },
  ];
            ...
            <Box component="table" sx={{ width: "100%", borderCollapse: "collapse" }}>
              <Box component="thead">
                <Box component="tr" sx={{ borderBottom: `1px solid ${tokens.colors.border.default}` }}>
                  {["订单号", "客户", "金额", "状态"].map((header) => (
                    <Box component="th" key={header} sx={{ textAlign: "left", py: s.sm, px: s.md, fontWeight: 500, color: "text.secondary", fontSize: "0.875rem" }}>
                      {header}
                    </Box>
                  ))}
                </Box>
                    ...
                    <Box component="td" sx={{ py: s.sm, px: s.md }}>
                      <Box
                        ...
                        sx={{
                          ...
                          fontSize: "0.75rem",
                          fontWeight: 500,
                          bgcolor: order.status === "待发货" ? "rgba(239, 68, 68, 0.1)" : "rgba(16, 185, 129, 0.1)",
                          color: order.status === "待发货" ? tokens.colors.accent.red : tokens.colors.accent.green,
                        }}
                      >
                        {order.status}
                      </Box>
