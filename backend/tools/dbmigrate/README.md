...
原生 Helm 命令显式使用 `--timeout 12m`，覆盖 Job 的 10 分钟期限。Argo 自愈也会重跑
幂等迁移；若 hook 卡住，不要选择性同步跳过它。先停止自动同步/新发布，核对 Job 是否仍在
运行及各服务实际版本；确认新 schema 与旧应用兼容后，保留能识别当前 migration 的已验证
迁移 digest，只回退应用 digest，再执行完整同步。不要用旧迁移镜像自动执行 `down`。
网络或 CLI 中断不等于 SQL 已停止，必须读取 Job 终态后再安排重试或回退。

迁移网络当前只允许集群内 `postgresql/pg-main` 与 DNS；prod 可以使用其中独立的 database，
若改用另一个实例，须先成对修改网络策略并验证，不以临时放通全网处理超时。
