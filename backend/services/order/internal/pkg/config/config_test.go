...
func TestGetConfigFromConsul(t *testing.T) {
	const path = "ecommerce/order/dev.yml"
...
func TestGetConfigFromConsul_KeyNotFound(t *testing.T) {
	...
	_, err := GetConfigFromConsul(newConsulClient(t, f.addr), "ecommerce/order/missing.yml")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "ecommerce/order/missing.yml")
}
...
func TestGetConfigFromConsul_Unreachable(t *testing.T) {
	...
	_, err := GetConfigFromConsul(newConsulClient(t, "127.0.0.1:1"), "ecommerce/order/dev.yml")
	require.Error(t, err)
...
func TestInit_Unreachable(t *testing.T) {
	...
	t.Setenv("CONSUL_PATH", "ecommerce/order/dev.yml")
	t.Setenv("CONSUL_SCHEME", "http")
