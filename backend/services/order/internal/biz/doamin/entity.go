...
// 系统按商家拆单，生成一个 OrderGroup 和多个 OrderRoot (子订单)
...
// 一个 OrderGroup 包含多个 OrderRoot，每个 OrderRoot 对应一个商家
type OrderRoot struct {
	Id             int64              // 订单自增主键ID
	OrderNo        string             // 订单号，业务唯一标识，如：OM202605090001
	GroupNo        string             // 关联订单组号 (业务关联，对应 OrderGroup.GroupNo)
	MerchantId     int64              // 商家ID (数据隔离核心字段)
	MerchantName   string             // 商家名称快照 (保留下单时的名称，防止后续商家改名)
	UserId         string             // 下单用户ID，来自Casdoor
	OrderItems     []OrderItem        // 订单明细列表 (聚合根包含子实体)
	OrderStatus    OrderStatusEnum    // 订单主状态
	ShippingStatus ShippingStatusEnum // 订单物流状态
...
type OrderLog struct {
	...
	MerchantId   int64            // 商家ID (数据隔离)
	OldStatus    *OrderStatusEnum // 变更前状态 (指针类型，创建订单时为空)
	NewStatus    OrderStatusEnum  // 变更后状态
	OperatorType OperatorTypeEnum // 操作者类型
...
type OrderRepo interface {
	...
	SaveOrderGroup(ctx context.Context, group *OrderGroup) error
	// GetOrderGroupByNo 根据订单组号查询
	GetOrderGroupByNo(ctx context.Context, groupNo string) (*OrderGroup, error)
	...
	UpdateOrderStatus(ctx context.Context, orderID int64, oldStatus *OrderStatusEnum, newStatus OrderStatusEnum, log *OrderLog) error
