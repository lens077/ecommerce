-- 订单服务初始结构。
-- 2026-08-21 由 schema/order.sql 原样转为 goose 版本化迁移（DDL 语义未动）；
-- 存量环境先 `dbmigrate -svc order baseline` 记账。
-- 原文件 search_path 带 products 只是历史残留（本文件未引用 products 对象），迁移中省去。
...
-- +goose Down
DROP TABLE IF EXISTS orders.order_log;
DROP TABLE IF EXISTS orders.order_item;
DROP TABLE IF EXISTS orders.order_main;
DROP TABLE IF EXISTS orders.order_group;
DROP TYPE IF EXISTS orders.shipping_status_enum;
DROP TYPE IF EXISTS orders.order_status_enum;
