...
-- 存量环境先 `dbmigrate -svc address baseline` 记账。
-- regions 字典数据仍由 ../seed/seed_regions.sql（cmd/regionseed 生成物）经 psql 整表重灌，
-- 不进 goose 种子——16k 行生成文件带自己的 BEGIN/TRUNCATE 事务语义，保持原路径。
...
-- +goose Down
DROP TABLE IF EXISTS addresses.regions;
