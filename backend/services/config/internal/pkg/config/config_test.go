...
func TestGetConfigFromConsul(t *testing.T) {
	const path = "ecommerce/config/dev.yml"
...
func TestGetConfigFromConsul_KeyNotFound(t *testing.T) {
	...
	_, err := GetConfigFromConsul(newConsulClient(t, f.addr), "ecommerce/config/missing.yml")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "ecommerce/config/missing.yml")
}
...
func TestGetConfigFromConsul_Unreachable(t *testing.T) {
	...
	_, err := GetConfigFromConsul(newConsulClient(t, "127.0.0.1:1"), "ecommerce/config/dev.yml")
	require.Error(t, err)
...
func TestInit_Unreachable(t *testing.T) {
	...
	t.Setenv("CONSUL_PATH", "ecommerce/config/dev.yml")
	t.Setenv("CONSUL_SCHEME", "http")
