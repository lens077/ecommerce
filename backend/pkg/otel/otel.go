...
// Options is the provider-neutral OpenTelemetry configuration consumed by this package.
type Options struct {
	Enabled bool
	Trace   TraceOptions
...
// SetupOTelSDK initializes trace, metric, and log export pipelines.
func SetupOTelSDK(ctx context.Context, info meta.AppInfo, options Options, logger *zap.Logger) (func(context.Context) error, error) {
	...
	meterProvider, err := newMeterProvider(ctx, res, options.Metric, logger)
	if err != nil {
		return shutdown, errors.Join(err, shutdown(ctx))
	}
...
func newMeterProvider(ctx context.Context, res *resource.Resource, options MetricOptions, logger *zap.Logger) (*metric.MeterProvider, error) {
