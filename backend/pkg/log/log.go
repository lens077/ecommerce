...
// Projector maps a service-owned Bootstrap into logging Options.
...
// FxLogger configures Fx's own event logger from the service Bootstrap.
...
func newLogger(options Options, info meta.AppInfo) (*zap.Logger, zap.AtomicLevel) {
	...
	if options.Format == constants.FormatConsole {
	...
	return zap.New(zapcore.NewTee(stdoutCore, otelCore), zap.AddCaller()), level
}
