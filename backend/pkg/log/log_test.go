...
func TestNewLoggerFiltersLevels(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.level, func(t *testing.T) {
			output := logAllLevels(t, test.level, constants.FormatJson)
...
func TestNewLoggerInvalidLevelFallsBackToDebug(t *testing.T) {
	assert.Contains(t, logAllLevels(t, "invalid-level", constants.FormatJson), "msg-debug")
}

func TestNewLoggerEmptyLevelIsInfo(t *testing.T) {
	output := logAllLevels(t, "", constants.FormatJson)
...
func TestNewLoggerJSONFormat(t *testing.T) {
	output := captureStdout(t, func() {
		NewLogger(options("info", constants.FormatJson), testAppInfo).
...
func TestNewLoggerConsoleFormat(t *testing.T) {
	output := captureStdout(t, func() {
		NewLogger(options("info", constants.FormatConsole), testAppInfo).Info("hello")
	})
...
func logProject(message *wrapperspb.StringValue) Options {
	return options(message.GetValue(), constants.FormatJson)
}
