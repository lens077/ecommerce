// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: api/casdoor/v1/user.proto

package casdoorv1

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// ManagedAccount 托管账户
// 用于 SSO 或凭证管理
type ManagedAccount struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Application   string                 `protobuf:"bytes,1,opt,name=application,proto3" json:"application,omitempty"`
	Username      string                 `protobuf:"bytes,2,opt,name=username,proto3" json:"username,omitempty"`
	Password      string                 `protobuf:"bytes,3,opt,name=password,proto3" json:"password,omitempty"` // 敏感字段，通常不应该在 API 响应中出现
	SigninUrl     string                 `protobuf:"bytes,4,opt,name=signin_url,json=signinUrl,proto3" json:"signin_url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ManagedAccount) Reset() {
	*x = ManagedAccount{}
	mi := &file_api_casdoor_v1_user_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ManagedAccount) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ManagedAccount) ProtoMessage() {}

func (x *ManagedAccount) ProtoReflect() protoreflect.Message {
	mi := &file_api_casdoor_v1_user_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ManagedAccount.ProtoReflect.Descriptor instead.
func (*ManagedAccount) Descriptor() ([]byte, []int) {
	return file_api_casdoor_v1_user_proto_rawDescGZIP(), []int{0}
}

func (x *ManagedAccount) GetApplication() string {
	if x != nil {
		return x.Application
	}
	return ""
}

func (x *ManagedAccount) GetUsername() string {
	if x != nil {
		return x.Username
	}
	return ""
}

func (x *ManagedAccount) GetPassword() string {
	if x != nil {
		return x.Password
	}
	return ""
}

func (x *ManagedAccount) GetSigninUrl() string {
	if x != nil {
		return x.SigninUrl
	}
	return ""
}

// Role 角色
// Casdoor 的角色定义，包含了权限和用户组的嵌套引用
type Role struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 基础信息
	Owner       string `protobuf:"bytes,1,opt,name=owner,proto3" json:"owner,omitempty"`
	Name        string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	CreatedTime string `protobuf:"bytes,3,opt,name=created_time,json=createdTime,proto3" json:"created_time,omitempty"`
	DisplayName string `protobuf:"bytes,4,opt,name=display_name,json=displayName,proto3" json:"display_name,omitempty"`
	Description string `protobuf:"bytes,5,opt,name=description,proto3" json:"description,omitempty"`
	// 关系 (使用 repeated string 来表示 Go 中的 []string)
	Users         []string `protobuf:"bytes,6,rep,name=users,proto3" json:"users,omitempty"`
	Groups        []string `protobuf:"bytes,7,rep,name=groups,proto3" json:"groups,omitempty"`
	Roles         []string `protobuf:"bytes,8,rep,name=roles,proto3" json:"roles,omitempty"` // 角色的嵌套定义
	Domains       []string `protobuf:"bytes,9,rep,name=domains,proto3" json:"domains,omitempty"`
	IsEnabled     bool     `protobuf:"varint,10,opt,name=is_enabled,json=isEnabled,proto3" json:"is_enabled,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Role) Reset() {
	*x = Role{}
	mi := &file_api_casdoor_v1_user_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Role) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Role) ProtoMessage() {}

func (x *Role) ProtoReflect() protoreflect.Message {
	mi := &file_api_casdoor_v1_user_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Role.ProtoReflect.Descriptor instead.
func (*Role) Descriptor() ([]byte, []int) {
	return file_api_casdoor_v1_user_proto_rawDescGZIP(), []int{1}
}

func (x *Role) GetOwner() string {
	if x != nil {
		return x.Owner
	}
	return ""
}

func (x *Role) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Role) GetCreatedTime() string {
	if x != nil {
		return x.CreatedTime
	}
	return ""
}

func (x *Role) GetDisplayName() string {
	if x != nil {
		return x.DisplayName
	}
	return ""
}

func (x *Role) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *Role) GetUsers() []string {
	if x != nil {
		return x.Users
	}
	return nil
}

func (x *Role) GetGroups() []string {
	if x != nil {
		return x.Groups
	}
	return nil
}

func (x *Role) GetRoles() []string {
	if x != nil {
		return x.Roles
	}
	return nil
}

func (x *Role) GetDomains() []string {
	if x != nil {
		return x.Domains
	}
	return nil
}

func (x *Role) GetIsEnabled() bool {
	if x != nil {
		return x.IsEnabled
	}
	return false
}

// Permission 权限
// Casdoor 的权限定义，详细描述了资源和操作
type Permission struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 基础信息
	Owner       string `protobuf:"bytes,1,opt,name=owner,proto3" json:"owner,omitempty"`
	Name        string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	CreatedTime string `protobuf:"bytes,3,opt,name=created_time,json=createdTime,proto3" json:"created_time,omitempty"`
	DisplayName string `protobuf:"bytes,4,opt,name=display_name,json=displayName,proto3" json:"display_name,omitempty"`
	Description string `protobuf:"bytes,5,opt,name=description,proto3" json:"description,omitempty"`
	// 关系
	Users   []string `protobuf:"bytes,6,rep,name=users,proto3" json:"users,omitempty"`
	Groups  []string `protobuf:"bytes,7,rep,name=groups,proto3" json:"groups,omitempty"`
	Roles   []string `protobuf:"bytes,8,rep,name=roles,proto3" json:"roles,omitempty"`
	Domains []string `protobuf:"bytes,9,rep,name=domains,proto3" json:"domains,omitempty"`
	// 权限模型 (RBAC)
	Model        string   `protobuf:"bytes,10,opt,name=model,proto3" json:"model,omitempty"`
	Adapter      string   `protobuf:"bytes,11,opt,name=adapter,proto3" json:"adapter,omitempty"`
	ResourceType string   `protobuf:"bytes,12,opt,name=resource_type,json=resourceType,proto3" json:"resource_type,omitempty"`
	Resources    []string `protobuf:"bytes,13,rep,name=resources,proto3" json:"resources,omitempty"`
	Actions      []string `protobuf:"bytes,14,rep,name=actions,proto3" json:"actions,omitempty"`
	Effect       string   `protobuf:"bytes,15,opt,name=effect,proto3" json:"effect,omitempty"`
	IsEnabled    bool     `protobuf:"varint,16,opt,name=is_enabled,json=isEnabled,proto3" json:"is_enabled,omitempty"`
	// 审批流程 (如果启用)
	Submitter     string `protobuf:"bytes,17,opt,name=submitter,proto3" json:"submitter,omitempty"`
	Approver      string `protobuf:"bytes,18,opt,name=approver,proto3" json:"approver,omitempty"`
	ApproveTime   string `protobuf:"bytes,19,opt,name=approve_time,json=approveTime,proto3" json:"approve_time,omitempty"`
	State         string `protobuf:"bytes,20,opt,name=state,proto3" json:"state,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Permission) Reset() {
	*x = Permission{}
	mi := &file_api_casdoor_v1_user_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Permission) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Permission) ProtoMessage() {}

func (x *Permission) ProtoReflect() protoreflect.Message {
	mi := &file_api_casdoor_v1_user_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Permission.ProtoReflect.Descriptor instead.
func (*Permission) Descriptor() ([]byte, []int) {
	return file_api_casdoor_v1_user_proto_rawDescGZIP(), []int{2}
}

func (x *Permission) GetOwner() string {
	if x != nil {
		return x.Owner
	}
	return ""
}

func (x *Permission) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Permission) GetCreatedTime() string {
	if x != nil {
		return x.CreatedTime
	}
	return ""
}

func (x *Permission) GetDisplayName() string {
	if x != nil {
		return x.DisplayName
	}
	return ""
}

func (x *Permission) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *Permission) GetUsers() []string {
	if x != nil {
		return x.Users
	}
	return nil
}

func (x *Permission) GetGroups() []string {
	if x != nil {
		return x.Groups
	}
	return nil
}

func (x *Permission) GetRoles() []string {
	if x != nil {
		return x.Roles
	}
	return nil
}

func (x *Permission) GetDomains() []string {
	if x != nil {
		return x.Domains
	}
	return nil
}

func (x *Permission) GetModel() string {
	if x != nil {
		return x.Model
	}
	return ""
}

func (x *Permission) GetAdapter() string {
	if x != nil {
		return x.Adapter
	}
	return ""
}

func (x *Permission) GetResourceType() string {
	if x != nil {
		return x.ResourceType
	}
	return ""
}

func (x *Permission) GetResources() []string {
	if x != nil {
		return x.Resources
	}
	return nil
}

func (x *Permission) GetActions() []string {
	if x != nil {
		return x.Actions
	}
	return nil
}

func (x *Permission) GetEffect() string {
	if x != nil {
		return x.Effect
	}
	return ""
}

func (x *Permission) GetIsEnabled() bool {
	if x != nil {
		return x.IsEnabled
	}
	return false
}

func (x *Permission) GetSubmitter() string {
	if x != nil {
		return x.Submitter
	}
	return ""
}

func (x *Permission) GetApprover() string {
	if x != nil {
		return x.Approver
	}
	return ""
}

func (x *Permission) GetApproveTime() string {
	if x != nil {
		return x.ApproveTime
	}
	return ""
}

func (x *Permission) GetState() string {
	if x != nil {
		return x.State
	}
	return ""
}

// 此结构体作为与 Casdoor SDK 交互的完整数据模型
type User struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 基础信息 (主键和时间戳)
	Owner       string `protobuf:"bytes,1,opt,name=owner,proto3" json:"owner,omitempty"`
	Name        string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	CreatedTime string `protobuf:"bytes,3,opt,name=created_time,json=createdTime,proto3" json:"created_time,omitempty"` // 推荐改为 google.protobuf.Timestamp
	UpdatedTime string `protobuf:"bytes,4,opt,name=updated_time,json=updatedTime,proto3" json:"updated_time,omitempty"` // 推荐改为 google.protobuf.Timestamp
	// 核心身份信息
	Id           string `protobuf:"bytes,5,opt,name=id,proto3" json:"id,omitempty"`
	ExternalId   string `protobuf:"bytes,6,opt,name=external_id,json=externalId,proto3" json:"external_id,omitempty"`
	Type         string `protobuf:"bytes,7,opt,name=type,proto3" json:"type,omitempty"`
	Password     string `protobuf:"bytes,8,opt,name=password,proto3" json:"password,omitempty"`
	PasswordSalt string `protobuf:"bytes,9,opt,name=password_salt,json=passwordSalt,proto3" json:"password_salt,omitempty"`
	PasswordType string `protobuf:"bytes,10,opt,name=password_type,json=passwordType,proto3" json:"password_type,omitempty"`
	// 个人资料
	DisplayName     string `protobuf:"bytes,11,opt,name=display_name,json=displayName,proto3" json:"display_name,omitempty"`
	FirstName       string `protobuf:"bytes,12,opt,name=first_name,json=firstName,proto3" json:"first_name,omitempty"`
	LastName        string `protobuf:"bytes,13,opt,name=last_name,json=lastName,proto3" json:"last_name,omitempty"`
	Avatar          string `protobuf:"bytes,14,opt,name=avatar,proto3" json:"avatar,omitempty"`
	AvatarType      string `protobuf:"bytes,15,opt,name=avatar_type,json=avatarType,proto3" json:"avatar_type,omitempty"`
	PermanentAvatar string `protobuf:"bytes,16,opt,name=permanent_avatar,json=permanentAvatar,proto3" json:"permanent_avatar,omitempty"`
	Email           string `protobuf:"bytes,17,opt,name=email,proto3" json:"email,omitempty"`
	EmailVerified   bool   `protobuf:"varint,18,opt,name=email_verified,json=emailVerified,proto3" json:"email_verified,omitempty"`
	Phone           string `protobuf:"bytes,19,opt,name=phone,proto3" json:"phone,omitempty"`
	CountryCode     string `protobuf:"bytes,20,opt,name=country_code,json=countryCode,proto3" json:"country_code,omitempty"`
	Region          string `protobuf:"bytes,21,opt,name=region,proto3" json:"region,omitempty"`
	Location        string `protobuf:"bytes,22,opt,name=location,proto3" json:"location,omitempty"`
	// Go 结构体中是 []string，Proto 中是 repeated string
	Address     []string `protobuf:"bytes,23,rep,name=address,proto3" json:"address,omitempty"`
	Affiliation string   `protobuf:"bytes,24,opt,name=affiliation,proto3" json:"affiliation,omitempty"`
	Title       string   `protobuf:"bytes,25,opt,name=title,proto3" json:"title,omitempty"`
	IdCardType  string   `protobuf:"bytes,26,opt,name=id_card_type,json=idCardType,proto3" json:"id_card_type,omitempty"`
	IdCard      string   `protobuf:"bytes,27,opt,name=id_card,json=idCard,proto3" json:"id_card,omitempty"`
	Homepage    string   `protobuf:"bytes,28,opt,name=homepage,proto3" json:"homepage,omitempty"`
	Bio         string   `protobuf:"bytes,29,opt,name=bio,proto3" json:"bio,omitempty"`
	Tag         string   `protobuf:"bytes,30,opt,name=tag,proto3" json:"tag,omitempty"`
	Language    string   `protobuf:"bytes,31,opt,name=language,proto3" json:"language,omitempty"`
	Gender      string   `protobuf:"bytes,32,opt,name=gender,proto3" json:"gender,omitempty"`
	Birthday    string   `protobuf:"bytes,33,opt,name=birthday,proto3" json:"birthday,omitempty"`
	Education   string   `protobuf:"bytes,34,opt,name=education,proto3" json:"education,omitempty"`
	Score       int32    `protobuf:"varint,35,opt,name=score,proto3" json:"score,omitempty"`
	Karma       int32    `protobuf:"varint,36,opt,name=karma,proto3" json:"karma,omitempty"`
	Ranking     int32    `protobuf:"varint,37,opt,name=ranking,proto3" json:"ranking,omitempty"`
	// 状态和权限
	IsDefaultAvatar   bool   `protobuf:"varint,38,opt,name=is_default_avatar,json=isDefaultAvatar,proto3" json:"is_default_avatar,omitempty"`
	IsOnline          bool   `protobuf:"varint,39,opt,name=is_online,json=isOnline,proto3" json:"is_online,omitempty"`
	IsAdmin           bool   `protobuf:"varint,40,opt,name=is_admin,json=isAdmin,proto3" json:"is_admin,omitempty"`
	IsForbidden       bool   `protobuf:"varint,41,opt,name=is_forbidden,json=isForbidden,proto3" json:"is_forbidden,omitempty"`
	IsDeleted         bool   `protobuf:"varint,42,opt,name=is_deleted,json=isDeleted,proto3" json:"is_deleted,omitempty"`
	SignupApplication string `protobuf:"bytes,43,opt,name=signup_application,json=signupApplication,proto3" json:"signup_application,omitempty"`
	Hash              string `protobuf:"bytes,44,opt,name=hash,proto3" json:"hash,omitempty"`
	PreHash           string `protobuf:"bytes,45,opt,name=pre_hash,json=preHash,proto3" json:"pre_hash,omitempty"`
	AccessKey         string `protobuf:"bytes,46,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	AccessSecret      string `protobuf:"bytes,47,opt,name=access_secret,json=accessSecret,proto3" json:"access_secret,omitempty"`
	// 登录记录
	CreatedIp      string `protobuf:"bytes,48,opt,name=created_ip,json=createdIp,proto3" json:"created_ip,omitempty"`
	LastSigninTime string `protobuf:"bytes,49,opt,name=last_signin_time,json=lastSigninTime,proto3" json:"last_signin_time,omitempty"`
	LastSigninIp   string `protobuf:"bytes,50,opt,name=last_signin_ip,json=lastSigninIp,proto3" json:"last_signin_ip,omitempty"`
	// OAuth/SSO 链接 (非常多的字段)
	Github          string `protobuf:"bytes,51,opt,name=github,proto3" json:"github,omitempty"`
	Google          string `protobuf:"bytes,52,opt,name=google,proto3" json:"google,omitempty"`
	Qq              string `protobuf:"bytes,53,opt,name=qq,proto3" json:"qq,omitempty"`
	WeChat          string `protobuf:"bytes,54,opt,name=we_chat,json=weChat,proto3" json:"we_chat,omitempty"`
	Facebook        string `protobuf:"bytes,55,opt,name=facebook,proto3" json:"facebook,omitempty"`
	DingTalk        string `protobuf:"bytes,56,opt,name=ding_talk,json=dingTalk,proto3" json:"ding_talk,omitempty"`
	Weibo           string `protobuf:"bytes,57,opt,name=weibo,proto3" json:"weibo,omitempty"`
	Gitee           string `protobuf:"bytes,58,opt,name=gitee,proto3" json:"gitee,omitempty"`
	LinkedIn        string `protobuf:"bytes,59,opt,name=linked_in,json=linkedIn,proto3" json:"linked_in,omitempty"`
	Wecom           string `protobuf:"bytes,60,opt,name=wecom,proto3" json:"wecom,omitempty"`
	Lark            string `protobuf:"bytes,61,opt,name=lark,proto3" json:"lark,omitempty"`
	Gitlab          string `protobuf:"bytes,62,opt,name=gitlab,proto3" json:"gitlab,omitempty"`
	Adfs            string `protobuf:"bytes,63,opt,name=adfs,proto3" json:"adfs,omitempty"`
	Baidu           string `protobuf:"bytes,64,opt,name=baidu,proto3" json:"baidu,omitempty"`
	Alipay          string `protobuf:"bytes,65,opt,name=alipay,proto3" json:"alipay,omitempty"`
	Casdoor         string `protobuf:"bytes,66,opt,name=casdoor,proto3" json:"casdoor,omitempty"`
	Infoflow        string `protobuf:"bytes,67,opt,name=infoflow,proto3" json:"infoflow,omitempty"`
	Apple           string `protobuf:"bytes,68,opt,name=apple,proto3" json:"apple,omitempty"`
	AzureAd         string `protobuf:"bytes,69,opt,name=azure_ad,json=azureAd,proto3" json:"azure_ad,omitempty"`
	Slack           string `protobuf:"bytes,70,opt,name=slack,proto3" json:"slack,omitempty"`
	Steam           string `protobuf:"bytes,71,opt,name=steam,proto3" json:"steam,omitempty"`
	Bilibili        string `protobuf:"bytes,72,opt,name=bilibili,proto3" json:"bilibili,omitempty"`
	Okta            string `protobuf:"bytes,73,opt,name=okta,proto3" json:"okta,omitempty"`
	Douyin          string `protobuf:"bytes,74,opt,name=douyin,proto3" json:"douyin,omitempty"`
	Line            string `protobuf:"bytes,75,opt,name=line,proto3" json:"line,omitempty"`
	Amazon          string `protobuf:"bytes,76,opt,name=amazon,proto3" json:"amazon,omitempty"`
	Auth0           string `protobuf:"bytes,77,opt,name=auth0,proto3" json:"auth0,omitempty"`
	BattleNet       string `protobuf:"bytes,78,opt,name=battle_net,json=battleNet,proto3" json:"battle_net,omitempty"`
	Bitbucket       string `protobuf:"bytes,79,opt,name=bitbucket,proto3" json:"bitbucket,omitempty"`
	Box             string `protobuf:"bytes,80,opt,name=box,proto3" json:"box,omitempty"`
	CloudFoundry    string `protobuf:"bytes,81,opt,name=cloud_foundry,json=cloudFoundry,proto3" json:"cloud_foundry,omitempty"`
	DailyMotion     string `protobuf:"bytes,82,opt,name=daily_motion,json=dailyMotion,proto3" json:"daily_motion,omitempty"`
	Deezer          string `protobuf:"bytes,83,opt,name=deezer,proto3" json:"deezer,omitempty"`
	DigitalOcean    string `protobuf:"bytes,84,opt,name=digital_ocean,json=digitalOcean,proto3" json:"digital_ocean,omitempty"`
	Discord         string `protobuf:"bytes,85,opt,name=discord,proto3" json:"discord,omitempty"`
	Dropbox         string `protobuf:"bytes,86,opt,name=dropbox,proto3" json:"dropbox,omitempty"`
	EveOnline       string `protobuf:"bytes,87,opt,name=eve_online,json=eveOnline,proto3" json:"eve_online,omitempty"`
	Fitbit          string `protobuf:"bytes,88,opt,name=fitbit,proto3" json:"fitbit,omitempty"`
	Gitea           string `protobuf:"bytes,89,opt,name=gitea,proto3" json:"gitea,omitempty"`
	Heroku          string `protobuf:"bytes,90,opt,name=heroku,proto3" json:"heroku,omitempty"`
	InfluxCloud     string `protobuf:"bytes,91,opt,name=influx_cloud,json=influxCloud,proto3" json:"influx_cloud,omitempty"`
	Instagram       string `protobuf:"bytes,92,opt,name=instagram,proto3" json:"instagram,omitempty"`
	Intercom        string `protobuf:"bytes,93,opt,name=intercom,proto3" json:"intercom,omitempty"`
	Kakao           string `protobuf:"bytes,94,opt,name=kakao,proto3" json:"kakao,omitempty"`
	Lastfm          string `protobuf:"bytes,95,opt,name=lastfm,proto3" json:"lastfm,omitempty"`
	Mailru          string `protobuf:"bytes,96,opt,name=mailru,proto3" json:"mailru,omitempty"`
	Meetup          string `protobuf:"bytes,97,opt,name=meetup,proto3" json:"meetup,omitempty"`
	MicrosoftOnline string `protobuf:"bytes,98,opt,name=microsoft_online,json=microsoftOnline,proto3" json:"microsoft_online,omitempty"`
	Naver           string `protobuf:"bytes,99,opt,name=naver,proto3" json:"naver,omitempty"`
	Nextcloud       string `protobuf:"bytes,100,opt,name=nextcloud,proto3" json:"nextcloud,omitempty"`
	OneDrive        string `protobuf:"bytes,101,opt,name=one_drive,json=oneDrive,proto3" json:"one_drive,omitempty"`
	Oura            string `protobuf:"bytes,102,opt,name=oura,proto3" json:"oura,omitempty"`
	Patreon         string `protobuf:"bytes,103,opt,name=patreon,proto3" json:"patreon,omitempty"`
	Paypal          string `protobuf:"bytes,104,opt,name=paypal,proto3" json:"paypal,omitempty"`
	SalesForce      string `protobuf:"bytes,105,opt,name=sales_force,json=salesForce,proto3" json:"sales_force,omitempty"`
	Shopify         string `protobuf:"bytes,106,opt,name=shopify,proto3" json:"shopify,omitempty"`
	Soundcloud      string `protobuf:"bytes,107,opt,name=soundcloud,proto3" json:"soundcloud,omitempty"`
	Spotify         string `protobuf:"bytes,108,opt,name=spotify,proto3" json:"spotify,omitempty"`
	Strava          string `protobuf:"bytes,109,opt,name=strava,proto3" json:"strava,omitempty"`
	Stripe          string `protobuf:"bytes,110,opt,name=stripe,proto3" json:"stripe,omitempty"`
	TikTok          string `protobuf:"bytes,111,opt,name=tik_tok,json=tikTok,proto3" json:"tik_tok,omitempty"`
	Tumblr          string `protobuf:"bytes,112,opt,name=tumblr,proto3" json:"tumblr,omitempty"`
	Twitch          string `protobuf:"bytes,113,opt,name=twitch,proto3" json:"twitch,omitempty"`
	Twitter         string `protobuf:"bytes,114,opt,name=twitter,proto3" json:"twitter,omitempty"`
	Typetalk        string `protobuf:"bytes,115,opt,name=typetalk,proto3" json:"typetalk,omitempty"`
	Uber            string `protobuf:"bytes,116,opt,name=uber,proto3" json:"uber,omitempty"`
	Vk              string `protobuf:"bytes,117,opt,name=vk,proto3" json:"vk,omitempty"`
	Wepay           string `protobuf:"bytes,118,opt,name=wepay,proto3" json:"wepay,omitempty"`
	Xero            string `protobuf:"bytes,119,opt,name=xero,proto3" json:"xero,omitempty"`
	Yahoo           string `protobuf:"bytes,120,opt,name=yahoo,proto3" json:"yahoo,omitempty"`
	Yammer          string `protobuf:"bytes,121,opt,name=yammer,proto3" json:"yammer,omitempty"`
	Yandex          string `protobuf:"bytes,122,opt,name=yandex,proto3" json:"yandex,omitempty"`
	Zoom            string `protobuf:"bytes,123,opt,name=zoom,proto3" json:"zoom,omitempty"`
	MetaMask        string `protobuf:"bytes,124,opt,name=meta_mask,json=metaMask,proto3" json:"meta_mask,omitempty"`
	Web3Onboard     string `protobuf:"bytes,125,opt,name=web3_onboard,json=web3Onboard,proto3" json:"web3_onboard,omitempty"`
	Custom          string `protobuf:"bytes,126,opt,name=custom,proto3" json:"custom,omitempty"`
	// MFA 和恢复
	PreferredMfaType string   `protobuf:"bytes,127,opt,name=preferred_mfa_type,json=preferredMfaType,proto3" json:"preferred_mfa_type,omitempty"`
	RecoveryCodes    []string `protobuf:"bytes,128,rep,name=recovery_codes,json=recoveryCodes,proto3" json:"recovery_codes,omitempty"`
	TotpSecret       string   `protobuf:"bytes,129,opt,name=totp_secret,json=totpSecret,proto3" json:"totp_secret,omitempty"`
	MfaPhoneEnabled  bool     `protobuf:"varint,130,opt,name=mfa_phone_enabled,json=mfaPhoneEnabled,proto3" json:"mfa_phone_enabled,omitempty"`
	MfaEmailEnabled  bool     `protobuf:"varint,131,opt,name=mfa_email_enabled,json=mfaEmailEnabled,proto3" json:"mfa_email_enabled,omitempty"`
	// 邀请
	Invitation     string `protobuf:"bytes,132,opt,name=invitation,proto3" json:"invitation,omitempty"`
	InvitationCode string `protobuf:"bytes,133,opt,name=invitation_code,json=invitationCode,proto3" json:"invitation_code,omitempty"`
	// LDAP 和属性
	Ldap string `protobuf:"bytes,134,opt,name=ldap,proto3" json:"ldap,omitempty"`
	// 映射 Go 的 map[string]string
	Properties map[string]string `protobuf:"bytes,135,rep,name=properties,proto3" json:"properties,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// 关系和时间戳
	Roles                  []*Role       `protobuf:"bytes,136,rep,name=roles,proto3" json:"roles,omitempty"`
	Permissions            []*Permission `protobuf:"bytes,137,rep,name=permissions,proto3" json:"permissions,omitempty"`
	Groups                 []string      `protobuf:"bytes,138,rep,name=groups,proto3" json:"groups,omitempty"`
	LastChangePasswordTime string        `protobuf:"bytes,139,opt,name=last_change_password_time,json=lastChangePasswordTime,proto3" json:"last_change_password_time,omitempty"`
	// 错误登录记录
	LastSigninWrongTime string `protobuf:"bytes,140,opt,name=last_signin_wrong_time,json=lastSigninWrongTime,proto3" json:"last_signin_wrong_time,omitempty"`
	SigninWrongTimes    int32  `protobuf:"varint,141,opt,name=signin_wrong_times,json=signinWrongTimes,proto3" json:"signin_wrong_times,omitempty"`
	// 托管账户和密码状态
	ManagedAccounts    []*ManagedAccount `protobuf:"bytes,142,rep,name=managed_accounts,json=managedAccounts,proto3" json:"managed_accounts,omitempty"`
	NeedUpdatePassword bool              `protobuf:"varint,143,opt,name=need_update_password,json=needUpdatePassword,proto3" json:"need_update_password,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *User) Reset() {
	*x = User{}
	mi := &file_api_casdoor_v1_user_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *User) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*User) ProtoMessage() {}

func (x *User) ProtoReflect() protoreflect.Message {
	mi := &file_api_casdoor_v1_user_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use User.ProtoReflect.Descriptor instead.
func (*User) Descriptor() ([]byte, []int) {
	return file_api_casdoor_v1_user_proto_rawDescGZIP(), []int{3}
}

func (x *User) GetOwner() string {
	if x != nil {
		return x.Owner
	}
	return ""
}

func (x *User) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *User) GetCreatedTime() string {
	if x != nil {
		return x.CreatedTime
	}
	return ""
}

func (x *User) GetUpdatedTime() string {
	if x != nil {
		return x.UpdatedTime
	}
	return ""
}

func (x *User) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *User) GetExternalId() string {
	if x != nil {
		return x.ExternalId
	}
	return ""
}

func (x *User) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *User) GetPassword() string {
	if x != nil {
		return x.Password
	}
	return ""
}

func (x *User) GetPasswordSalt() string {
	if x != nil {
		return x.PasswordSalt
	}
	return ""
}

func (x *User) GetPasswordType() string {
	if x != nil {
		return x.PasswordType
	}
	return ""
}

func (x *User) GetDisplayName() string {
	if x != nil {
		return x.DisplayName
	}
	return ""
}

func (x *User) GetFirstName() string {
	if x != nil {
		return x.FirstName
	}
	return ""
}

func (x *User) GetLastName() string {
	if x != nil {
		return x.LastName
	}
	return ""
}

func (x *User) GetAvatar() string {
	if x != nil {
		return x.Avatar
	}
	return ""
}

func (x *User) GetAvatarType() string {
	if x != nil {
		return x.AvatarType
	}
	return ""
}

func (x *User) GetPermanentAvatar() string {
	if x != nil {
		return x.PermanentAvatar
	}
	return ""
}

func (x *User) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

func (x *User) GetEmailVerified() bool {
	if x != nil {
		return x.EmailVerified
	}
	return false
}

func (x *User) GetPhone() string {
	if x != nil {
		return x.Phone
	}
	return ""
}

func (x *User) GetCountryCode() string {
	if x != nil {
		return x.CountryCode
	}
	return ""
}

func (x *User) GetRegion() string {
	if x != nil {
		return x.Region
	}
	return ""
}

func (x *User) GetLocation() string {
	if x != nil {
		return x.Location
	}
	return ""
}

func (x *User) GetAddress() []string {
	if x != nil {
		return x.Address
	}
	return nil
}

func (x *User) GetAffiliation() string {
	if x != nil {
		return x.Affiliation
	}
	return ""
}

func (x *User) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

func (x *User) GetIdCardType() string {
	if x != nil {
		return x.IdCardType
	}
	return ""
}

func (x *User) GetIdCard() string {
	if x != nil {
		return x.IdCard
	}
	return ""
}

func (x *User) GetHomepage() string {
	if x != nil {
		return x.Homepage
	}
	return ""
}

func (x *User) GetBio() string {
	if x != nil {
		return x.Bio
	}
	return ""
}

func (x *User) GetTag() string {
	if x != nil {
		return x.Tag
	}
	return ""
}

func (x *User) GetLanguage() string {
	if x != nil {
		return x.Language
	}
	return ""
}

func (x *User) GetGender() string {
	if x != nil {
		return x.Gender
	}
	return ""
}

func (x *User) GetBirthday() string {
	if x != nil {
		return x.Birthday
	}
	return ""
}

func (x *User) GetEducation() string {
	if x != nil {
		return x.Education
	}
	return ""
}

func (x *User) GetScore() int32 {
	if x != nil {
		return x.Score
	}
	return 0
}

func (x *User) GetKarma() int32 {
	if x != nil {
		return x.Karma
	}
	return 0
}

func (x *User) GetRanking() int32 {
	if x != nil {
		return x.Ranking
	}
	return 0
}

func (x *User) GetIsDefaultAvatar() bool {
	if x != nil {
		return x.IsDefaultAvatar
	}
	return false
}

func (x *User) GetIsOnline() bool {
	if x != nil {
		return x.IsOnline
	}
	return false
}

func (x *User) GetIsAdmin() bool {
	if x != nil {
		return x.IsAdmin
	}
	return false
}

func (x *User) GetIsForbidden() bool {
	if x != nil {
		return x.IsForbidden
	}
	return false
}

func (x *User) GetIsDeleted() bool {
	if x != nil {
		return x.IsDeleted
	}
	return false
}

func (x *User) GetSignupApplication() string {
	if x != nil {
		return x.SignupApplication
	}
	return ""
}

func (x *User) GetHash() string {
	if x != nil {
		return x.Hash
	}
	return ""
}

func (x *User) GetPreHash() string {
	if x != nil {
		return x.PreHash
	}
	return ""
}

func (x *User) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *User) GetAccessSecret() string {
	if x != nil {
		return x.AccessSecret
	}
	return ""
}

func (x *User) GetCreatedIp() string {
	if x != nil {
		return x.CreatedIp
	}
	return ""
}

func (x *User) GetLastSigninTime() string {
	if x != nil {
		return x.LastSigninTime
	}
	return ""
}

func (x *User) GetLastSigninIp() string {
	if x != nil {
		return x.LastSigninIp
	}
	return ""
}

func (x *User) GetGithub() string {
	if x != nil {
		return x.Github
	}
	return ""
}

func (x *User) GetGoogle() string {
	if x != nil {
		return x.Google
	}
	return ""
}

func (x *User) GetQq() string {
	if x != nil {
		return x.Qq
	}
	return ""
}

func (x *User) GetWeChat() string {
	if x != nil {
		return x.WeChat
	}
	return ""
}

func (x *User) GetFacebook() string {
	if x != nil {
		return x.Facebook
	}
	return ""
}

func (x *User) GetDingTalk() string {
	if x != nil {
		return x.DingTalk
	}
	return ""
}

func (x *User) GetWeibo() string {
	if x != nil {
		return x.Weibo
	}
	return ""
}

func (x *User) GetGitee() string {
	if x != nil {
		return x.Gitee
	}
	return ""
}

func (x *User) GetLinkedIn() string {
	if x != nil {
		return x.LinkedIn
	}
	return ""
}

func (x *User) GetWecom() string {
	if x != nil {
		return x.Wecom
	}
	return ""
}

func (x *User) GetLark() string {
	if x != nil {
		return x.Lark
	}
	return ""
}

func (x *User) GetGitlab() string {
	if x != nil {
		return x.Gitlab
	}
	return ""
}

func (x *User) GetAdfs() string {
	if x != nil {
		return x.Adfs
	}
	return ""
}

func (x *User) GetBaidu() string {
	if x != nil {
		return x.Baidu
	}
	return ""
}

func (x *User) GetAlipay() string {
	if x != nil {
		return x.Alipay
	}
	return ""
}

func (x *User) GetCasdoor() string {
	if x != nil {
		return x.Casdoor
	}
	return ""
}

func (x *User) GetInfoflow() string {
	if x != nil {
		return x.Infoflow
	}
	return ""
}

func (x *User) GetApple() string {
	if x != nil {
		return x.Apple
	}
	return ""
}

func (x *User) GetAzureAd() string {
	if x != nil {
		return x.AzureAd
	}
	return ""
}

func (x *User) GetSlack() string {
	if x != nil {
		return x.Slack
	}
	return ""
}

func (x *User) GetSteam() string {
	if x != nil {
		return x.Steam
	}
	return ""
}

func (x *User) GetBilibili() string {
	if x != nil {
		return x.Bilibili
	}
	return ""
}

func (x *User) GetOkta() string {
	if x != nil {
		return x.Okta
	}
	return ""
}

func (x *User) GetDouyin() string {
	if x != nil {
		return x.Douyin
	}
	return ""
}

func (x *User) GetLine() string {
	if x != nil {
		return x.Line
	}
	return ""
}

func (x *User) GetAmazon() string {
	if x != nil {
		return x.Amazon
	}
	return ""
}

func (x *User) GetAuth0() string {
	if x != nil {
		return x.Auth0
	}
	return ""
}

func (x *User) GetBattleNet() string {
	if x != nil {
		return x.BattleNet
	}
	return ""
}

func (x *User) GetBitbucket() string {
	if x != nil {
		return x.Bitbucket
	}
	return ""
}

func (x *User) GetBox() string {
	if x != nil {
		return x.Box
	}
	return ""
}

func (x *User) GetCloudFoundry() string {
	if x != nil {
		return x.CloudFoundry
	}
	return ""
}

func (x *User) GetDailyMotion() string {
	if x != nil {
		return x.DailyMotion
	}
	return ""
}

func (x *User) GetDeezer() string {
	if x != nil {
		return x.Deezer
	}
	return ""
}

func (x *User) GetDigitalOcean() string {
	if x != nil {
		return x.DigitalOcean
	}
	return ""
}

func (x *User) GetDiscord() string {
	if x != nil {
		return x.Discord
	}
	return ""
}

func (x *User) GetDropbox() string {
	if x != nil {
		return x.Dropbox
	}
	return ""
}

func (x *User) GetEveOnline() string {
	if x != nil {
		return x.EveOnline
	}
	return ""
}

func (x *User) GetFitbit() string {
	if x != nil {
		return x.Fitbit
	}
	return ""
}

func (x *User) GetGitea() string {
	if x != nil {
		return x.Gitea
	}
	return ""
}

func (x *User) GetHeroku() string {
	if x != nil {
		return x.Heroku
	}
	return ""
}

func (x *User) GetInfluxCloud() string {
	if x != nil {
		return x.InfluxCloud
	}
	return ""
}

func (x *User) GetInstagram() string {
	if x != nil {
		return x.Instagram
	}
	return ""
}

func (x *User) GetIntercom() string {
	if x != nil {
		return x.Intercom
	}
	return ""
}

func (x *User) GetKakao() string {
	if x != nil {
		return x.Kakao
	}
	return ""
}

func (x *User) GetLastfm() string {
	if x != nil {
		return x.Lastfm
	}
	return ""
}

func (x *User) GetMailru() string {
	if x != nil {
		return x.Mailru
	}
	return ""
}

func (x *User) GetMeetup() string {
	if x != nil {
		return x.Meetup
	}
	return ""
}

func (x *User) GetMicrosoftOnline() string {
	if x != nil {
		return x.MicrosoftOnline
	}
	return ""
}

func (x *User) GetNaver() string {
	if x != nil {
		return x.Naver
	}
	return ""
}

func (x *User) GetNextcloud() string {
	if x != nil {
		return x.Nextcloud
	}
	return ""
}

func (x *User) GetOneDrive() string {
	if x != nil {
		return x.OneDrive
	}
	return ""
}

func (x *User) GetOura() string {
	if x != nil {
		return x.Oura
	}
	return ""
}

func (x *User) GetPatreon() string {
	if x != nil {
		return x.Patreon
	}
	return ""
}

func (x *User) GetPaypal() string {
	if x != nil {
		return x.Paypal
	}
	return ""
}

func (x *User) GetSalesForce() string {
	if x != nil {
		return x.SalesForce
	}
	return ""
}

func (x *User) GetShopify() string {
	if x != nil {
		return x.Shopify
	}
	return ""
}

func (x *User) GetSoundcloud() string {
	if x != nil {
		return x.Soundcloud
	}
	return ""
}

func (x *User) GetSpotify() string {
	if x != nil {
		return x.Spotify
	}
	return ""
}

func (x *User) GetStrava() string {
	if x != nil {
		return x.Strava
	}
	return ""
}

func (x *User) GetStripe() string {
	if x != nil {
		return x.Stripe
	}
	return ""
}

func (x *User) GetTikTok() string {
	if x != nil {
		return x.TikTok
	}
	return ""
}

func (x *User) GetTumblr() string {
	if x != nil {
		return x.Tumblr
	}
	return ""
}

func (x *User) GetTwitch() string {
	if x != nil {
		return x.Twitch
	}
	return ""
}

func (x *User) GetTwitter() string {
	if x != nil {
		return x.Twitter
	}
	return ""
}

func (x *User) GetTypetalk() string {
	if x != nil {
		return x.Typetalk
	}
	return ""
}

func (x *User) GetUber() string {
	if x != nil {
		return x.Uber
	}
	return ""
}

func (x *User) GetVk() string {
	if x != nil {
		return x.Vk
	}
	return ""
}

func (x *User) GetWepay() string {
	if x != nil {
		return x.Wepay
	}
	return ""
}

func (x *User) GetXero() string {
	if x != nil {
		return x.Xero
	}
	return ""
}

func (x *User) GetYahoo() string {
	if x != nil {
		return x.Yahoo
	}
	return ""
}

func (x *User) GetYammer() string {
	if x != nil {
		return x.Yammer
	}
	return ""
}

func (x *User) GetYandex() string {
	if x != nil {
		return x.Yandex
	}
	return ""
}

func (x *User) GetZoom() string {
	if x != nil {
		return x.Zoom
	}
	return ""
}

func (x *User) GetMetaMask() string {
	if x != nil {
		return x.MetaMask
	}
	return ""
}

func (x *User) GetWeb3Onboard() string {
	if x != nil {
		return x.Web3Onboard
	}
	return ""
}

func (x *User) GetCustom() string {
	if x != nil {
		return x.Custom
	}
	return ""
}

func (x *User) GetPreferredMfaType() string {
	if x != nil {
		return x.PreferredMfaType
	}
	return ""
}

func (x *User) GetRecoveryCodes() []string {
	if x != nil {
		return x.RecoveryCodes
	}
	return nil
}

func (x *User) GetTotpSecret() string {
	if x != nil {
		return x.TotpSecret
	}
	return ""
}

func (x *User) GetMfaPhoneEnabled() bool {
	if x != nil {
		return x.MfaPhoneEnabled
	}
	return false
}

func (x *User) GetMfaEmailEnabled() bool {
	if x != nil {
		return x.MfaEmailEnabled
	}
	return false
}

func (x *User) GetInvitation() string {
	if x != nil {
		return x.Invitation
	}
	return ""
}

func (x *User) GetInvitationCode() string {
	if x != nil {
		return x.InvitationCode
	}
	return ""
}

func (x *User) GetLdap() string {
	if x != nil {
		return x.Ldap
	}
	return ""
}

func (x *User) GetProperties() map[string]string {
	if x != nil {
		return x.Properties
	}
	return nil
}

func (x *User) GetRoles() []*Role {
	if x != nil {
		return x.Roles
	}
	return nil
}

func (x *User) GetPermissions() []*Permission {
	if x != nil {
		return x.Permissions
	}
	return nil
}

func (x *User) GetGroups() []string {
	if x != nil {
		return x.Groups
	}
	return nil
}

func (x *User) GetLastChangePasswordTime() string {
	if x != nil {
		return x.LastChangePasswordTime
	}
	return ""
}

func (x *User) GetLastSigninWrongTime() string {
	if x != nil {
		return x.LastSigninWrongTime
	}
	return ""
}

func (x *User) GetSigninWrongTimes() int32 {
	if x != nil {
		return x.SigninWrongTimes
	}
	return 0
}

func (x *User) GetManagedAccounts() []*ManagedAccount {
	if x != nil {
		return x.ManagedAccounts
	}
	return nil
}

func (x *User) GetNeedUpdatePassword() bool {
	if x != nil {
		return x.NeedUpdatePassword
	}
	return false
}

var File_api_casdoor_v1_user_proto protoreflect.FileDescriptor

const file_api_casdoor_v1_user_proto_rawDesc = "" +
	"\n" +
	"\x19api/casdoor/v1/user.proto\x12\n" +
	"casdoor.v1\x1a#third_party/validate/validate.proto\"\xb1\x01\n" +
	"\x0eManagedAccount\x12*\n" +
	"\vapplication\x18\x01 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\vapplication\x12$\n" +
	"\busername\x18\x02 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\busername\x12$\n" +
	"\bpassword\x18\x03 \x01(\tB\b\xbaH\x05r\x03\x18\x80 R\bpassword\x12'\n" +
	"\n" +
	"signin_url\x18\x04 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\tsigninUrl\"\x8e\x03\n" +
	"\x04Role\x12\x1e\n" +
	"\x05owner\x18\x01 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x05owner\x12\x1c\n" +
	"\x04name\x18\x02 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x04name\x12*\n" +
	"\fcreated_time\x18\x03 \x01(\tB\a\xbaH\x04r\x02\x18@R\vcreatedTime\x12+\n" +
	"\fdisplay_name\x18\x04 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\vdisplayName\x12*\n" +
	"\vdescription\x18\x05 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\vdescription\x12&\n" +
	"\x05users\x18\x06 \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\x05users\x12(\n" +
	"\x06groups\x18\a \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\x06groups\x12&\n" +
	"\x05roles\x18\b \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\x05roles\x12*\n" +
	"\adomains\x18\t \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\adomains\x12\x1d\n" +
	"\n" +
	"is_enabled\x18\n" +
	" \x01(\bR\tisEnabled\"\x9d\x06\n" +
	"\n" +
	"Permission\x12\x1e\n" +
	"\x05owner\x18\x01 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x05owner\x12\x1c\n" +
	"\x04name\x18\x02 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x04name\x12*\n" +
	"\fcreated_time\x18\x03 \x01(\tB\a\xbaH\x04r\x02\x18@R\vcreatedTime\x12+\n" +
	"\fdisplay_name\x18\x04 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\vdisplayName\x12*\n" +
	"\vdescription\x18\x05 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\vdescription\x12&\n" +
	"\x05users\x18\x06 \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\x05users\x12(\n" +
	"\x06groups\x18\a \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\x06groups\x12&\n" +
	"\x05roles\x18\b \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\x05roles\x12*\n" +
	"\adomains\x18\t \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\adomains\x12\x1e\n" +
	"\x05model\x18\n" +
	" \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x05model\x12\"\n" +
	"\aadapter\x18\v \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\aadapter\x12-\n" +
	"\rresource_type\x18\f \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\fresourceType\x12.\n" +
	"\tresources\x18\r \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x10R\tresources\x12*\n" +
	"\aactions\x18\x0e \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\aactions\x12\x1f\n" +
	"\x06effect\x18\x0f \x01(\tB\a\xbaH\x04r\x02\x18@R\x06effect\x12\x1d\n" +
	"\n" +
	"is_enabled\x18\x10 \x01(\bR\tisEnabled\x12&\n" +
	"\tsubmitter\x18\x11 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\tsubmitter\x12$\n" +
	"\bapprover\x18\x12 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\bapprover\x12*\n" +
	"\fapprove_time\x18\x13 \x01(\tB\a\xbaH\x04r\x02\x18@R\vapproveTime\x12\x1d\n" +
	"\x05state\x18\x14 \x01(\tB\a\xbaH\x04r\x02\x18@R\x05state\"\xec+\n" +
	"\x04User\x12\x1e\n" +
	"\x05owner\x18\x01 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x05owner\x12\x1c\n" +
	"\x04name\x18\x02 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x04name\x12*\n" +
	"\fcreated_time\x18\x03 \x01(\tB\a\xbaH\x04r\x02\x18@R\vcreatedTime\x12*\n" +
	"\fupdated_time\x18\x04 \x01(\tB\a\xbaH\x04r\x02\x18@R\vupdatedTime\x12\x18\n" +
	"\x02id\x18\x05 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x02id\x12)\n" +
	"\vexternal_id\x18\x06 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\n" +
	"externalId\x12\x1b\n" +
	"\x04type\x18\a \x01(\tB\a\xbaH\x04r\x02\x18@R\x04type\x12$\n" +
	"\bpassword\x18\b \x01(\tB\b\xbaH\x05r\x03\x18\x80 R\bpassword\x12-\n" +
	"\rpassword_salt\x18\t \x01(\tB\b\xbaH\x05r\x03\x18\x80\bR\fpasswordSalt\x12,\n" +
	"\rpassword_type\x18\n" +
	" \x01(\tB\a\xbaH\x04r\x02\x18@R\fpasswordType\x12+\n" +
	"\fdisplay_name\x18\v \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\vdisplayName\x12'\n" +
	"\n" +
	"first_name\x18\f \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\tfirstName\x12%\n" +
	"\tlast_name\x18\r \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\blastName\x12 \n" +
	"\x06avatar\x18\x0e \x01(\tB\b\xbaH\x05r\x03\x18\x80@R\x06avatar\x12(\n" +
	"\vavatar_type\x18\x0f \x01(\tB\a\xbaH\x04r\x02\x18@R\n" +
	"avatarType\x123\n" +
	"\x10permanent_avatar\x18\x10 \x01(\tB\b\xbaH\x05r\x03\x18\x80@R\x0fpermanentAvatar\x12\x1e\n" +
	"\x05email\x18\x11 \x01(\tB\b\xbaH\x05r\x03\x18\xc0\x02R\x05email\x12%\n" +
	"\x0eemail_verified\x18\x12 \x01(\bR\remailVerified\x12\x1d\n" +
	"\x05phone\x18\x13 \x01(\tB\a\xbaH\x04r\x02\x18 R\x05phone\x12*\n" +
	"\fcountry_code\x18\x14 \x01(\tB\a\xbaH\x04r\x02\x18\bR\vcountryCode\x12 \n" +
	"\x06region\x18\x15 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x06region\x12$\n" +
	"\blocation\x18\x16 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\blocation\x12)\n" +
	"\aaddress\x18\x17 \x03(\tB\x0f\xbaH\f\x92\x01\t\x10d\"\x05r\x03\x18\x80\x04R\aaddress\x12*\n" +
	"\vaffiliation\x18\x18 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\vaffiliation\x12\x1e\n" +
	"\x05title\x18\x19 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\x05title\x12)\n" +
	"\fid_card_type\x18\x1a \x01(\tB\a\xbaH\x04r\x02\x18@R\n" +
	"idCardType\x12!\n" +
	"\aid_card\x18\x1b \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x06idCard\x12$\n" +
	"\bhomepage\x18\x1c \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\bhomepage\x12\x1a\n" +
	"\x03bio\x18\x1d \x01(\tB\b\xbaH\x05r\x03\x18\x80 R\x03bio\x12\x1a\n" +
	"\x03tag\x18\x1e \x01(\tB\b\xbaH\x05r\x03\x18\x80\x04R\x03tag\x12#\n" +
	"\blanguage\x18\x1f \x01(\tB\a\xbaH\x04r\x02\x18 R\blanguage\x12\x1f\n" +
	"\x06gender\x18  \x01(\tB\a\xbaH\x04r\x02\x18 R\x06gender\x12#\n" +
	"\bbirthday\x18! \x01(\tB\a\xbaH\x04r\x02\x18 R\bbirthday\x12&\n" +
	"\teducation\x18\" \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\teducation\x12\x1d\n" +
	"\x05score\x18# \x01(\x05B\a\xbaH\x04\x1a\x02(\x00R\x05score\x12\x1d\n" +
	"\x05karma\x18$ \x01(\x05B\a\xbaH\x04\x1a\x02(\x00R\x05karma\x12!\n" +
	"\aranking\x18% \x01(\x05B\a\xbaH\x04\x1a\x02(\x00R\aranking\x12*\n" +
	"\x11is_default_avatar\x18& \x01(\bR\x0fisDefaultAvatar\x12\x1b\n" +
	"\tis_online\x18' \x01(\bR\bisOnline\x12\x19\n" +
	"\bis_admin\x18( \x01(\bR\aisAdmin\x12!\n" +
	"\fis_forbidden\x18) \x01(\bR\visForbidden\x12\x1d\n" +
	"\n" +
	"is_deleted\x18* \x01(\bR\tisDeleted\x127\n" +
	"\x12signup_application\x18+ \x01(\tB\b\xbaH\x05r\x03\x18\x80\x01R\x11signupApplication\x12\x1c\n" +
	"\x04hash\x18, \x01(\tB\b\xbaH\x05r\x03\x18\x80\bR\x04hash\x12#\n" +
	"\bpre_hash\x18- \x01(\tB\b\xbaH\x05r\x03\x18\x80\bR\apreHash\x12'\n" +
	"\n" +
	"access_key\x18. \x01(\tB\b\xbaH\x05r\x03\x18\x80\bR\taccessKey\x12-\n" +
	"\raccess_secret\x18/ \x01(\tB\b\xbaH\x05r\x03\x18\x80 R\faccessSecret\x12&\n" +
	"\n" +
	"created_ip\x180 \x01(\tB\a\xbaH\x04r\x02\x18-R\tcreatedIp\x121\n" +
	"\x10last_signin_time\x181 \x01(\tB\a\xbaH\x04r\x02\x18@R\x0elastSigninTime\x12-\n" +
	"\x0elast_signin_ip\x182 \x01(\tB\a\xbaH\x04r\x02\x18-R\flastSigninIp\x12 \n" +
	"\x06github\x183 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06github\x12 \n" +
	"\x06google\x184 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06google\x12\x18\n" +
	"\x02qq\x185 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x02qq\x12!\n" +
	"\awe_chat\x186 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06weChat\x12$\n" +
	"\bfacebook\x187 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\bfacebook\x12%\n" +
	"\tding_talk\x188 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\bdingTalk\x12\x1e\n" +
	"\x05weibo\x189 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05weibo\x12\x1e\n" +
	"\x05gitee\x18: \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05gitee\x12%\n" +
	"\tlinked_in\x18; \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\blinkedIn\x12\x1e\n" +
	"\x05wecom\x18< \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05wecom\x12\x1c\n" +
	"\x04lark\x18= \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x04lark\x12 \n" +
	"\x06gitlab\x18> \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06gitlab\x12\x1c\n" +
	"\x04adfs\x18? \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x04adfs\x12\x1e\n" +
	"\x05baidu\x18@ \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05baidu\x12 \n" +
	"\x06alipay\x18A \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06alipay\x12\"\n" +
	"\acasdoor\x18B \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\acasdoor\x12$\n" +
	"\binfoflow\x18C \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\binfoflow\x12\x1e\n" +
	"\x05apple\x18D \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05apple\x12#\n" +
	"\bazure_ad\x18E \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\aazureAd\x12\x1e\n" +
	"\x05slack\x18F \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05slack\x12\x1e\n" +
	"\x05steam\x18G \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05steam\x12$\n" +
	"\bbilibili\x18H \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\bbilibili\x12\x1c\n" +
	"\x04okta\x18I \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x04okta\x12 \n" +
	"\x06douyin\x18J \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06douyin\x12\x1c\n" +
	"\x04line\x18K \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x04line\x12 \n" +
	"\x06amazon\x18L \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06amazon\x12\x1e\n" +
	"\x05auth0\x18M \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05auth0\x12'\n" +
	"\n" +
	"battle_net\x18N \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\tbattleNet\x12&\n" +
	"\tbitbucket\x18O \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\tbitbucket\x12\x1a\n" +
	"\x03box\x18P \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x03box\x12-\n" +
	"\rcloud_foundry\x18Q \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\fcloudFoundry\x12+\n" +
	"\fdaily_motion\x18R \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\vdailyMotion\x12 \n" +
	"\x06deezer\x18S \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06deezer\x12-\n" +
	"\rdigital_ocean\x18T \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\fdigitalOcean\x12\"\n" +
	"\adiscord\x18U \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\adiscord\x12\"\n" +
	"\adropbox\x18V \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\adropbox\x12'\n" +
	"\n" +
	"eve_online\x18W \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\teveOnline\x12 \n" +
	"\x06fitbit\x18X \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06fitbit\x12\x1e\n" +
	"\x05gitea\x18Y \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05gitea\x12 \n" +
	"\x06heroku\x18Z \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06heroku\x12+\n" +
	"\finflux_cloud\x18[ \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\vinfluxCloud\x12&\n" +
	"\tinstagram\x18\\ \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\tinstagram\x12$\n" +
	"\bintercom\x18] \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\bintercom\x12\x1e\n" +
	"\x05kakao\x18^ \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05kakao\x12 \n" +
	"\x06lastfm\x18_ \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06lastfm\x12 \n" +
	"\x06mailru\x18` \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06mailru\x12 \n" +
	"\x06meetup\x18a \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06meetup\x123\n" +
	"\x10microsoft_online\x18b \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x0fmicrosoftOnline\x12\x1e\n" +
	"\x05naver\x18c \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05naver\x12&\n" +
	"\tnextcloud\x18d \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\tnextcloud\x12%\n" +
	"\tone_drive\x18e \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\boneDrive\x12\x1c\n" +
	"\x04oura\x18f \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x04oura\x12\"\n" +
	"\apatreon\x18g \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\apatreon\x12 \n" +
	"\x06paypal\x18h \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06paypal\x12)\n" +
	"\vsales_force\x18i \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\n" +
	"salesForce\x12\"\n" +
	"\ashopify\x18j \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\ashopify\x12(\n" +
	"\n" +
	"soundcloud\x18k \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\n" +
	"soundcloud\x12\"\n" +
	"\aspotify\x18l \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\aspotify\x12 \n" +
	"\x06strava\x18m \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06strava\x12 \n" +
	"\x06stripe\x18n \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06stripe\x12!\n" +
	"\atik_tok\x18o \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06tikTok\x12 \n" +
	"\x06tumblr\x18p \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06tumblr\x12 \n" +
	"\x06twitch\x18q \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06twitch\x12\"\n" +
	"\atwitter\x18r \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\atwitter\x12$\n" +
	"\btypetalk\x18s \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\btypetalk\x12\x1c\n" +
	"\x04uber\x18t \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x04uber\x12\x18\n" +
	"\x02vk\x18u \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x02vk\x12\x1e\n" +
	"\x05wepay\x18v \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05wepay\x12\x1c\n" +
	"\x04xero\x18w \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x04xero\x12\x1e\n" +
	"\x05yahoo\x18x \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x05yahoo\x12 \n" +
	"\x06yammer\x18y \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06yammer\x12 \n" +
	"\x06yandex\x18z \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x06yandex\x12\x1c\n" +
	"\x04zoom\x18{ \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x04zoom\x12%\n" +
	"\tmeta_mask\x18| \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\bmetaMask\x12+\n" +
	"\fweb3_onboard\x18} \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\vweb3Onboard\x12 \n" +
	"\x06custom\x18~ \x01(\tB\b\xbaH\x05r\x03\x18\x80 R\x06custom\x125\n" +
	"\x12preferred_mfa_type\x18\x7f \x01(\tB\a\xbaH\x04r\x02\x18@R\x10preferredMfaType\x127\n" +
	"\x0erecovery_codes\x18\x80\x01 \x03(\tB\x0f\xbaH\f\x92\x01\t\x10d\"\x05r\x03\x18\x80\x02R\rrecoveryCodes\x12*\n" +
	"\vtotp_secret\x18\x81\x01 \x01(\tB\b\xbaH\x05r\x03\x18\x80\bR\n" +
	"totpSecret\x12+\n" +
	"\x11mfa_phone_enabled\x18\x82\x01 \x01(\bR\x0fmfaPhoneEnabled\x12+\n" +
	"\x11mfa_email_enabled\x18\x83\x01 \x01(\bR\x0fmfaEmailEnabled\x12)\n" +
	"\n" +
	"invitation\x18\x84\x01 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\n" +
	"invitation\x122\n" +
	"\x0finvitation_code\x18\x85\x01 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x02R\x0einvitationCode\x12\x1d\n" +
	"\x04ldap\x18\x86\x01 \x01(\tB\b\xbaH\x05r\x03\x18\x80\x10R\x04ldap\x12[\n" +
	"\n" +
	"properties\x18\x87\x01 \x03(\v2 .casdoor.v1.User.PropertiesEntryB\x18\xbaH\x15\x9a\x01\x12\x10d\"\ar\x05\x10\x01\x18\x80\x01*\x05r\x03\x18\x80 R\n" +
	"properties\x122\n" +
	"\x05roles\x18\x88\x01 \x03(\v2\x10.casdoor.v1.RoleB\t\xbaH\x06\x92\x01\x03\x10\xe8\aR\x05roles\x12D\n" +
	"\vpermissions\x18\x89\x01 \x03(\v2\x16.casdoor.v1.PermissionB\t\xbaH\x06\x92\x01\x03\x10\xe8\aR\vpermissions\x12)\n" +
	"\x06groups\x18\x8a\x01 \x03(\tB\x10\xbaH\r\x92\x01\n" +
	"\x10\xe8\a\"\x05r\x03\x18\x80\x02R\x06groups\x12C\n" +
	"\x19last_change_password_time\x18\x8b\x01 \x01(\tB\a\xbaH\x04r\x02\x18@R\x16lastChangePasswordTime\x12=\n" +
	"\x16last_signin_wrong_time\x18\x8c\x01 \x01(\tB\a\xbaH\x04r\x02\x18@R\x13lastSigninWrongTime\x126\n" +
	"\x12signin_wrong_times\x18\x8d\x01 \x01(\x05B\a\xbaH\x04\x1a\x02(\x00R\x10signinWrongTimes\x12P\n" +
	"\x10managed_accounts\x18\x8e\x01 \x03(\v2\x1a.casdoor.v1.ManagedAccountB\b\xbaH\x05\x92\x01\x02\x10dR\x0fmanagedAccounts\x121\n" +
	"\x14need_update_password\x18\x8f\x01 \x01(\bR\x12needUpdatePassword\x1a=\n" +
	"\x0fPropertiesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01B\xa3\x01\n" +
	"\x0ecom.casdoor.v1B\tUserProtoP\x01Z=github.com/lens077/ecommerce/backend/api/casdoor/v1;casdoorv1\xa2\x02\x03CXX\xaa\x02\n" +
	"Casdoor.V1\xca\x02\n" +
	"Casdoor\\V1\xe2\x02\x16Casdoor\\V1\\GPBMetadata\xea\x02\vCasdoor::V1b\x06proto3"

var (
	file_api_casdoor_v1_user_proto_rawDescOnce sync.Once
	file_api_casdoor_v1_user_proto_rawDescData []byte
)

func file_api_casdoor_v1_user_proto_rawDescGZIP() []byte {
	file_api_casdoor_v1_user_proto_rawDescOnce.Do(func() {
		file_api_casdoor_v1_user_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_api_casdoor_v1_user_proto_rawDesc), len(file_api_casdoor_v1_user_proto_rawDesc)))
	})
	return file_api_casdoor_v1_user_proto_rawDescData
}

var file_api_casdoor_v1_user_proto_msgTypes = make([]protoimpl.MessageInfo, 5)
var file_api_casdoor_v1_user_proto_goTypes = []any{
	(*ManagedAccount)(nil), // 0: casdoor.v1.ManagedAccount
	(*Role)(nil),           // 1: casdoor.v1.Role
	(*Permission)(nil),     // 2: casdoor.v1.Permission
	(*User)(nil),           // 3: casdoor.v1.User
	nil,                    // 4: casdoor.v1.User.PropertiesEntry
}
var file_api_casdoor_v1_user_proto_depIdxs = []int32{
	4, // 0: casdoor.v1.User.properties:type_name -> casdoor.v1.User.PropertiesEntry
	1, // 1: casdoor.v1.User.roles:type_name -> casdoor.v1.Role
	2, // 2: casdoor.v1.User.permissions:type_name -> casdoor.v1.Permission
	0, // 3: casdoor.v1.User.managed_accounts:type_name -> casdoor.v1.ManagedAccount
	4, // [4:4] is the sub-list for method output_type
	4, // [4:4] is the sub-list for method input_type
	4, // [4:4] is the sub-list for extension type_name
	4, // [4:4] is the sub-list for extension extendee
	0, // [0:4] is the sub-list for field type_name
}

func init() { file_api_casdoor_v1_user_proto_init() }
func file_api_casdoor_v1_user_proto_init() {
	if File_api_casdoor_v1_user_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_api_casdoor_v1_user_proto_rawDesc), len(file_api_casdoor_v1_user_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   5,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_api_casdoor_v1_user_proto_goTypes,
		DependencyIndexes: file_api_casdoor_v1_user_proto_depIdxs,
		MessageInfos:      file_api_casdoor_v1_user_proto_msgTypes,
	}.Build()
	File_api_casdoor_v1_user_proto = out.File
	file_api_casdoor_v1_user_proto_goTypes = nil
	file_api_casdoor_v1_user_proto_depIdxs = nil
}
