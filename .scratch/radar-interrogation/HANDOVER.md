...
2. **术语表**：初版由本会话建立，后被并发会话重构吸收；重构后按约定累计**新增 21 条**（存储与备份 5、Silo/社区分叉 2、OLTP/列存/ClickHouse 3、身份与凭据 7、密封与解封 1、影子双跑/fail-open 与 fail-close/JWKS 3）、**就地补充 3 处**（Silo 状态、ClickHouse 裁决、密封态 API 全拒细节）。
...
8. **§4 执行层深读**（用户「详细阅读」二连；此前一轮「继续 ②⑧⑪」被中断、未产生执行）：读全三份实施档案原文——D1（第 1 轮凭据次序化争论与合流：治理修订是 (c) 的硬前置）、T5（第 2 轮 OpenFGA 完整形态，雷达行外增量：128–200m/256Mi、check-cache+模型版本化缓存+撤权清缓存、写路径永远实查、ListObjects/BatchCheck 预取、owner==subject 本地短路）、R3-A 定稿（第 3 轮通过+3 补丁：dump 校验和 / CSP·XFO·Set-Cookie 头 diff 冒烟 / NTP 前置；主轴=公网 origin 不变消除全员重登）。产出：②⑧⑪ 依赖图、执行就绪度评估、**PITR 折扣发现**（casdoor 收编动机之一「纳入 CNPG PITR」因 §10 备份三件套 ⏸ 暂缓而打折，其余三条动机仍成立）。纯阅读，无决策变更。
...
| **三个零风险起手式（下一步首选）**：②(b) 治理修订稿（纯文档 ≤0.5 人日）/ ⑧ P0 查证（ssh 只读）/ ⑪ FGA 模型设计稿（读代码+建模），三者互不依赖可并行 | 本会话 §4 深读报告 |
| casdoor 收编的「PITR 纳管」动机被 §10 ⏸ 打折（收编后暂无异地 PITR；另三条动机成立，结论不翻） | radar §4 附 + 总览 §10 行 |
...
- 其余锚点未跑——本会话未动 Go/前端代码；提交前按 AGENTS.md 锚点执行（注意工作区混有并发会话的后端大改，那部分的验证责任在其会话）。AGENTS.md 会话中两次热更新（CI 改 tag 触发；新增 `scripts/verify-quick.sh` 默认锚点），以最新版为准。

## 7. 提问轨迹（全轮次）

| # | 用户提问 | 结果 |
|---|---|---|
| 1 | 审问 §2：Meilisearch+pgvector 为什么/是什么/解决什么/之前如何/单组件行不行 | 讲解，确立六问答题模板 |
| 2 | 建术语表并自动维护；typo 容错/facet/sortable 是什么 | `docs/GLOSSARY.md` 建立 + 讲解 |
| 3 | Meili 不存数据从 PG 读？pgvector 是倒排索引？ | 纠偏（写入喂/查询不回源；向量索引 ≠ 倒排）；发现术语表被并发重构为全项目版 |
| 4 | MinIO 和 SeaweedFS 呢？ | §10.6 讲解（替换 + 新增备份靶双角色）+ 术语表存储节 |
| 5 | pgsty/silo 分叉接管 MinIO，重新评审 | 实测复审：主结论维持、silo 🟡、论据降级；揪出「镜像谱系漏查」评审漏洞 |
| 6 | 按硬规则 5 沉淀；node2 是否切 silo | 沉淀 `tech-selection.md`；你拍板「切并立即执行」→ 生产切换完成（踩 `HOME=/tmp` 坑、公网 500 约 3 分钟、修复后验收全绿） |
| 7 | 重审 ClickHouse：1G 收益是否高于占用 | 复审账 → 你拍板「触发式缓上」→ 五处真相源落笔 |
| 8 | 详读 §4：每个工具意思 + 如何运作 | 七工具四问题 + 三动线讲解；术语表 +7 |
| 9 | Vault 是否也要 unseal？ | 是，同源设计；原理 + auto-unseal 取舍 |
| 10 | 不解封依旧正常，除非替换密钥值？ | 对一半：密封期 API 读写全拒；坏路径=「世界需要新值」；失效面清单 |
| 11 | 总结会话，给交接文档 | HANDOVER v1 |
| 12 | 继续 ②⑧⑪ | **该轮被中断，零执行**（唯一未完成轮，后由第 13 轮深读承接） |
| 13 | 详读 §4（二连） | 执行层深读：D1/T5/R3-A 原文 + ②⑧⑪ 依赖图 + 就绪度 + PITR 折扣发现；术语表 +3 |
| 14 | 总结：完成/ToDo/留意 | 总结 + HANDOVER 更新 |
| 15 | 全量总结（含提问轨迹） | 本节 + 最终总结 |
