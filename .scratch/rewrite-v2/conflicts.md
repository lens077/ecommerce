...
不是现役的 node3 Pigsty（211.144.221.229）——同名两机，见 C9。
...
| 出处 | 问题 |
|---|---|
| `.service-matrix.yaml:65` vs `:71-76` | 「Pigsty」一词指两台机器：`:65` 说「≠ Pigsty 那台 192.168.3.210」（旧局域网 Pigsty，已关机），`:71` 定义 node3 Pigsty（211.144.221.229）为现役。同文件内同名异指，未互相注明 |
| `docs/PRIORITY.md:257` | 「node3 二义」警示本身已过时：现在 node3 有第三个所指（211.144.221.229 的 Pigsty 机），警示只列了两个 |
